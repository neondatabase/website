...
First, ask me in one message (don't guess or pick a default): which org, an existing project in a supported region (`aws-us-east-2`, `aws-us-east-1`, `aws-eu-central-1`, or `aws-ap-southeast-1`) to reuse or a name to create a new one, and a one-line description of what I'm building. Neon's backend services (Functions, Object Storage, and the AI Gateway) are available in those four regions, on new or existing projects. Support is expanding toward all regions.
