import Sidebar from 'components/pages/blog/sidebar';
import Container from 'components/shared/container';
import { getAllCategories } from 'utils/api-wp';

// eslint-disable-next-line react/prop-types
const BlogPageLayout = async ({ children }) => {
  const categories = await getAllCategories();

  return (
    <div className="pt-20 safe-paddings pb-24 sm:pt-10 sm:pb-16 lg:pt-12 lg:pb-20">
      <Container className="flex flex-col" size="1600">
        <div className="flex gap-16 lg:flex-col lg:gap-0 xl:gap-3.5 xl:pl-0 md:relative">
          <Sidebar categories={categories} />
          <div className="relative w-full max-w-[1184px] lg:max-w-full">{children}</div>
        </div>
      </Container>
    </div>
  );
};

export default BlogPageLayout;
