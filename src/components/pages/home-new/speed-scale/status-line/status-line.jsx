...
const StatusLine = ({ className }) => (
  <Container className="2xl:max-w-[1344px] 2xl:pl-16 xl:max-w-5xl md:pl-5 md:pr-3" size="1600">
    <div
      className={cn(
        'flex h-4 items-center justify-between pl-40',
    ...
    >
      <span
        className="mr-2 h-3 w-2.5 shrink-0 bg-gray-new-60 xl:mr-1 xl:h-2.5 xl:w-2 lg:size-1.5"
        aria-hidden
      />
      <span className="mr-7 shrink-0 tracking-extra-tight xl:mr-4">
        SYSTEM: NEON DATABASE PLATFORM
      </span>
      <LinesPattern size={4} className="-mr-0.5 basis-11 xl:mr-0 xl:basis-14" />
      <LinesPattern size={8} className="basis-[175px] xl:basis-[240px]" />
      ...
      <LinesPattern size={18} className="basis-[106px] xl:basis-[104px] lg:hidden" />
      <LinesPattern size={8} className="basis-[170px] xl:hidden" />
      <LinesPattern size={4} className="basis-12 xl:hidden" />
      <span className="ml-[34px] shrink-0 xl:ml-0">[ CONNECTION: STABLE ]</span>
    </div>
  </Container>
);
