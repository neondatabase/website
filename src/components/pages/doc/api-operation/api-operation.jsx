...
import { toSdkMethodName } from 'utils/api-ref.mjs';
...
function buildInterfaces(operation) {
  const cliAvailable = !!(operation.cli?.command || operation.cli?.commands?.length);
  const cliCode = operation.cli?.command ?? operation.cli?.commands?.[0]?.command ?? null;
  return [
    { id: 'api', code: `${operation.method} ${operation.path}`, available: true },
    { id: 'cli', code: cliCode, available: cliAvailable },
    { id: 'sdk', code: `api.${toSdkMethodName(operation.operationId)}()`, available: true },
    { id: 'mcp', code: operation.mcp?.tool ?? null, available: !!operation.mcp?.tool },
    {
      id: 'console',
      code: operation.console?.breadcrumb ?? null,
      available: !!operation.console?.breadcrumb,
    },
  ];
}
...
const ApiOperation = ({ operation, breadcrumbs, navigationLinks, currentSlug }) => {
  ...
  const interfaces = buildInterfaces(operation);
