...
const Icon = ({ theme = 'default', className = null }) => {
  if (theme === 'docs') return <PageIcon className={clsx('top-[3px]', className)} aria-hidden />;
  if (theme === 'repo') return <GithubIcon className={clsx('top-[3px]', className)} aria-hidden />;
  return <CheckIcon className={clsx('top-1', className)} aria-hidden />;
};
      ...
      {React.Children.map(children, (child) => (
        <li
          className={clsx(
            ...
            '[&_a]:!text-inherit [&_a]:transition-colors [&_a]:duration-200',
            '[&_a:hover]:!text-secondary-8 [&_a:hover]:dark:!text-green-45'
          )}
        >
