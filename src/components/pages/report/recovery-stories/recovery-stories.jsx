...
const RecoveryStories = () => (
  <section className="recovery-stories safe-paddings mt-[155px]">
    <Container className="relative z-10 flex !max-w-[576px] flex-col items-start" size="xxs">
      <h2 className="font-title text-5xl font-medium leading-tight tracking-extra-tight">
        Recovery horror stories
      </h2>
      <ul className="mt-11 flex flex-col gap-y-[35px]">
        {DATA.map(({ description, role, company, team }, index) => (
          <li className="border-b border-dashed border-gray-new-15 pb-9" key={index}>
            <p className="text-lg leading-normal tracking-extra-tight text-gray-new-90">
              {description}
            </p>
            <div className="mt-3 flex items-center gap-x-2 text-sm leading-snug tracking-extra-tight">
              <p className="flex items-center gap-x-1 text-gray-new-60">
                <span>{role}</span>
                <span className="text-green-45">{company}</span>
              </p>
              <span className="text-gray-new-15">•</span>
