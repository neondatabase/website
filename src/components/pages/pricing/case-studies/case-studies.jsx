...
const CaseStudies = ({ className = null }) => (
  <section className={clsx('case-studies safe-paddings', className)}>
    <Container className="grid-gap-x grid grid-cols-12" size="medium">
      <div className="col-span-10 col-start-2 grid grid-cols-10 gap-x-10 rounded-2xl bg-black-new px-14 py-12 xl:col-span-full xl:col-start-1 xl:grid-cols-12 xl:items-center xl:gap-x-6 xl:p-8 lg:gap-x-4 lg:pb-10 lg:pt-9 md:py-8">
        <div className="col-span-4 col-start-1 flex flex-col items-start xl:max-w-[300px] xl:self-start xl:justify-self-start lg:col-span-full lg:max-w-none">
          <GradientLabel>Why Neon?</GradientLabel>
          <h2 className="mt-3 max-w-[340px] font-title text-[40px] font-medium leading-none tracking-[-0.02em] xl:max-w-[270px] xl:text-[44px] lg:max-w-none lg:text-4xl md:text-[32px]">
            Increase velocity. Scale efficiently.
          </h2>
          <p className="mt-4 max-w-[362px] text-lg font-light leading-snug xl:max-w-[280px] xl:text-base lg:max-w-[648px] md:mt-2.5 sm:max-w-none sm:pr-1.5">
            Teams of all sizes use branching to increase development velocity, and autoscaling means
            costs stay efficient with zero effort.
          </p>
          <Link
        ...
        <ul className="col-start-5 col-end-11 grid max-w-[608px] grid-cols-2 gap-x-[100px] gap-y-[38px] pl-10 2xl:gap-x-10 xl:col-start-6 xl:col-end-13 xl:max-w-none xl:gap-x-4 xl:pl-0 lg:col-span-full lg:mt-10 lg:gap-10 md:mt-8 md:grid-cols-1 md:gap-y-7">
          {items.map(({ icon, title, description }, index) => (
            <li className="max-w-[255px] xl:max-w-none" key={index}>
              <div className="flex items-center gap-x-4">
                <img
                  className="mt-0.5 h-[27px] shrink-0"
                  src={icon.src}
                  width={icon.width}
                  height={icon.height}
                  alt={icon.alt}
              ...
              <p className="mt-2.5 font-light leading-snug text-gray-new-60">{description}</p>
            </li>
          ))}
        </ul>
      </div>
