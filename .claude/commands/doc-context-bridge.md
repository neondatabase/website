...
- **Cursor skills (on-demand checks):** When the user asks for these tasks, follow the instructions in the corresponding skill file:
  - **Consistency check:** Read `.cursor/skills/consistency-check.md` — extract key procedural/definitional sentences, search for similar phrasing in `content/docs` (and optionally `content/guides`), report overlaps and suggest single source or aligned wording.
  - **docs-glossary-updater:** Read `.cursor/skills/docs-glossary-updater.md` — scan file(s) for Neon product terms, compare to `content/docs/reference/glossary.md`, output checklist of missing terms and terms to review.
