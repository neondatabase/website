...
- UI: A Beta version of Neon's _Autoscaling_ feature is now available for [paid plan](../introduction/billing#neon-plans) users in selected regions. Autoscaling allows you to specify a minimum and maximum compute size. Neon scales compute resources up and down within the specified compute size boundaries to meet workload demand. The Autoscaling feature can be enabled when [creating a Neon project](../manage/projects#create-a-project) or by [editing a compute endpoint](../manage/endpoints#edit-a-compute-endpoint).
