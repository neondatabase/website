...
[Pro plan](/docs/introduction/pro-plan) users can now define default compute settings for a Neon project. Settings include **Compute size**, **Autoscaling**, and **Auto-suspend delay**. To defines default compute settings, selecting **Compute** opens a **Default compute settings** page where you can set defaultThe default settings are used when creating a new branch or adding a compute endpoint to a branch via the Neon Console. However, you can override or change the **Default compute settings** when creating a branch.
