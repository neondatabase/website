...
- Safekeeper: Added support for downloading Write-ahead Logs (WAL) from S3 storage on demand.
- Safekeeper: Switched to etcd subscriptions to keep Pageservers up to date regarding safekeeper status. etcd is a key-value store for critical distributed system data.
