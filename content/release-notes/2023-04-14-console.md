...
- UI: Fixed the **Edit Compute Endpoint** dialog so that the **Autoscaling** compute provisioning slider does not allow selecting unsupported minimum values. Autoscaling has a minimum value of 1 vCPU and 4 GB of RAM.
- UI: Fixed an issue that caused Neon Dashboard text to overflow when reducing the size of the browser window.
