...
- Control Plane: Fixed a passwordless connection issue that occurred when creating a new project using the project selection interface. Neon's passwordless auth feature allows you to quickly connect to a new or existing Neon project. For more information, see [Connect with Neon's psql passwordless auth](/docs/get-started-with-neon/query-with-psql-editor/#connect-with-neons-psql-passwordless-auth).
