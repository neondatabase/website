...
- UI: Updated the Neon Free Tier limits. See [Technical Preview Free Tier](../introduction/technical-preview-free-tier/) for details. The new Neon Free Tier limits apply to both new and existing projects, but the previous 10 GB storage limit will be honored for existing projects that currently have more than 3 GB of data.
- UI: Added a **Delete endpoint** button to the **Endpoints** page.
- UI: Updated the Neon [passwordless auth](../connect/passwordless-connect/) feature to select an endpoint to connect to instead of a project.
