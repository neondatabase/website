...
- UI: Added support for fractional Compute Units (CU), enabling configuration of compute endpoints with a half or  quarter of a CU. One CU is 1 vCPU and 4GB of RAM.
...
- UI: Added a **Request subscription cancellation** button to the **Billing** page in the Neon Console, which opens an **Unsubscribe** dialog where a paid plan customer can submit a cancellation request.
- UI: Added a safety limit of 20 active compute endpoints for the Neon paid plans. When the limit is exceeded, an endpoint is created but not activated.
...
- UI: Revised cancellation warning text and related styles on the **Billing** page in the Neon Console.
