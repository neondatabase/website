...
- **Instagres (formerly Neon Launchpad)**  
  Instantly spin up a Postgres database with Instagres; no signup required. Perfect for workshops and rapid prototyping. Try it at [pg.new](https://pg.new/). **Update:** At the time of this release it was at neon.new; it's now at pg.new. To learn more, see our [Claimable Postgres docs](https://neon.com/docs/reference/claimable-postgres).

- **get-db (CLI)**  
  Bootstrap Neon Postgres database with the `get-db` CLI:  
  `npx get-db --yes`  
...
- **Vite Plugin for Neon**  
  ...
  `npm add -D vite-plugin-db`

**Update:** The CLI was `npx neondb` and the Vite plugin was `@neondatabase/vite-plugin-postgres` at the time of this release; they are now `npx get-db` and `vite-plugin-db`.
