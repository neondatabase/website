...
## TanStack Integration & open source tools for fast Java Script development

We're excited to annouce that Neon is now the official database partner of **TanStack**, and that we’ve released new tools to simplify Postgres integration across the TanStack and Vite ecosystems:

- **Create TanStack Add-on**  
  Instantly set up a fullstack application with a Neon Postgres database:
  `pnpm create tanstack --add-on neon`

- **Neon Launchpad**  
  Instantly spin up a Postgres database — no signup required. Perfect for workshops and rapid prototyping. Try it at [neon.new](https://neon.new/). To learn more about Neon Launchpad, see our [docs](https://neon.com/docs/reference/neon-launchpad).

- **NeonDB (CLI + SDK)**  
  Bootstrap Neon Postgres database with:  
  `npx neondb --yes`  
...
- **Vite Plugin for Neon**  
  ...
  Includes support for `.sql` seed files.
