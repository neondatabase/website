...
Neon Auth is available on all plans, including Free. Get started with [Next.js](/docs/auth/quick-start/nextjs-api-only), [React](/docs/auth/quick-start/react), or [TanStack](/docs/auth/quick-start/tanstack-router).
...
- The [Neon MCP Server](/docs/ai/neon-mcp-server) now supports the new Neon Auth with an updated `provision_neon_auth` tool and a new `setup-neon-auth` prompt, an interactive guide for setting up Neon Auth in Vite+React projects.
