...
Next week, we'll make a related change, and we wanted to give you a heads-up in case there any changes you need to make to your CI scripts or workflows. **When you perform a [branch reset or restore](/docs/introduction/point-in-time-restore) operation child branches, the passwords on those child branches will be preserved.** Currently, resetting or restoring a child branch (where passwords were previously regenerated) resets passwords back to those used on the parent branch.
