...
A few weeks ago, we introduced a new protected branches feature: When you create a child branch from a protected branch, new passwords are automatically generated for the Postgres roles on the child branch. This behavior prevents the exposure of passwords that could be used to access your protected branch. You can read more about this feature [here](/docs/guides/protected-branches#new-passwords-generated-for-postgres-roles-on-child-branches).

Next week, we'll make a related change, and we wanted to give you a heads-up in case there any changes you need to make to your CI scripts or workflows. **Coming next week: When you perform a [branch reset or restore](/docs/introduction/point-in-time-restore) operation child branches, the passwords on those child branches will be preserved.** Currently, resetting or restoring a child branch (where passwords were previously regenerated) resets passwords back to those used on the parent branch.
