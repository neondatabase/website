...
## `@neon/sdk` 1.0 — TypeScript SDK for the Neon API

Neon's official TypeScript SDK for the Platform API is now at 1.0. [`@neon/sdk`](https://www.npmjs.com/package/@neon/sdk) is a fetch-based replacement for [`@neondatabase/api-client`](https://www.npmjs.com/package/@neondatabase/api-client), with `createNeonClient` for ergonomic resource namespaces, typed `{ data, error }` results, and a full `raw` layer generated from Neon's OpenAPI spec. Version 1.0 unifies error handling across both layers and adds ergonomic wrappers for Neon Auth, project permissions, branch recovery, and beta platform APIs (storage, buckets, functions, credentials, and AI gateway).

```bash
npm install @neon/sdk
```

See the [TypeScript SDK documentation](/docs/reference/typescript-sdk) and [migration guide](/docs/reference/migrate-api-client-to-sdk) for quick start and upgrading from `@neondatabase/api-client`.
