...
If you have any questions, reach out to us in [Discord](https://discord.gg/N8kvBm7qH5).
...
- The **Quickstart** banner that appears at the top of the Neon **Dashboard** for newly created projects now includes commands for creating and listing branches using the [Neon CLI](/docs/cli).
...
- Updated the documentation link on the **Developer Settings** page (accessed from your account **Profile**) to link to the [Neon API Reference](/docs/reference/api) instead of the [Neon documentation landing page](/docs/introduction).
- Removed the requirement to specify a `--project-id` value when issuing a [Neon CLI ip-allow](/docs/cli/ip-allow) command when there is only a single Neon project in your Neon account. The option is now only required if you have multiple Neon projects.
