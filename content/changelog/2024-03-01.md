...
- The [Neon Vercel Integration](/docs/guides/vercel-overview) now displays a message on the **Deployment Details** page in Vercel under **Running checks** if you exceed the branch limit for your Neon project.
...
- The [Get projects](/docs/reference/api/projects/list-projects) and [Get shared projects](/docs/reference/api/projects/list-shared-projects) APIs now fetch all projects when using the `search` parameter. If a `limit` value is specified, it is applied after the fetch to limit displayed results.
