...
- **Neon MCP Server**
  ...
  - Added new tools for branch compute management, including `list_branch_computes` for viewing compute endpoints with details on compute ID, type, size, and autoscaling information.
  ...
  - Added a comprehensive query performance tuning system with multiple tools:
...
- **Neon API**
  ...
  - Enhanced snapshot functionality on the **Backup & Restore** page in the Neon Console to support archived branches.

- **Backup & Restore** (available in Early Access)

  - Previously, creating a snapshot of an archived branch would fail. Now, the branch is automatically unarchived before the snapshot is created.
