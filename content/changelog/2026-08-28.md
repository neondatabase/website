...
Installs the [Neon MCP Server](/docs/cli/mcp) into your agents by writing their MCP config for you. It points each agent at the hosted server, sets up authentication, and can narrow which tools the agent sees. By default it mints a Neon API key and writes it into the config. Optionally, you can pass the `--oauth` option to skip the key and have the agent sign in to Neon on first use instead.
