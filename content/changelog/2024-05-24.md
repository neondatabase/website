...
- The Neon CLI [connection-string](/docs/cli/connection-string) command and the Neon API [Get connection URI](/docs/reference/api/projects/get-connection-uri) method now generate a connection URI with a `postgresql://` scheme designator instead of the shorter `postgres://` designator. While both scheme designators are [valid](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING-URIS), we found the longer version to be more widely supported.
