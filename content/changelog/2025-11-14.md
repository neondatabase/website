...
- **Neon Console:**
  ...
  - Snapshot schedule improvements: added validation messages and logic to prevent invalid configurations, and updated the interface to explicitly display UTC for the snapshot hour. The **Edit schedule** button is now hidden for non-root branches since backup schedules can only be set on root branches.
