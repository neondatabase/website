...
You can also view and set a branch's automatic backup schedule with `neon snapshots schedule get` and `neon snapshots schedule set`. Automatic backup schedules are a beta feature; the CLI surfaces the API's message if they aren't enabled for your project. See the [`neon snapshots` reference](/docs/cli/snapshots) for the full list of subcommands.
...
<Admonition type="note" title="Set up the MCP server and agent skills in one shot">
This week's updates span the Neon CLI, the MCP server, and the [Neon backend](/docs/get-started/backend-overview) services. If you're not set up yet, connect the MCP server and install the Neon agent skills with a single command:

```bash
npx neon@latest init
```

It runs OAuth, creates an API key, configures the Neon MCP server for your editor, and installs the Neon agent skills. Run it from your project root, then ask your AI assistant to get started with Neon. See the [`neon init` reference](/docs/cli/init) for what gets created and where. To install the CLI itself, run `npm install -g neon`. For the backend beta services (Object Storage, Functions, and AI Gateway), follow the [backend beta guide](/docs/get-started/backend-beta).
</Admonition>
