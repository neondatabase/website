...
- You can now view the last active time for a compute endpoint associated with your branch, which is useful in determining which branches you have accessed recently. In the Neon Console, you can view the **Last active** time on the **Branches** page where all branches are listed or under **Computes** on individual branch pages. In the Neon API, the [Get a compute endpoint](https://api-docs.neon.tech/reference/getprojectendpoint) API response now includes a `last_active` field displaying a timestamp indicating when the compute endpoint was last active.
- The Neon SQL Editor now uses the Neon serverless driver for database connections. If you utilize the Neon [IP Allow](/docs/introduction/ip-allow) feature and your public IP address isn't currently on the allowlist, you may encounter a pop-up when you next query your database through the Neon SQL Editor. This pop-up will request permission to add your IP address to the allowlist. To identify your IP address, Neon employs the [ipify API](https://www.ipify.org/) Public IP Address API.
