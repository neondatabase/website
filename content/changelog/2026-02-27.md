...
When a new minor version is available on Neon, it is applied the next time your compute restarts. For more about how we handle Postgres version upgrades, refer to our [Postgres version support policy](/docs/postgresql/postgres-version-support).
...
We fixed an issue where deleting a database with [Neon Auth](/docs/auth/overview) left stale state and caused 500 errors when opening [Neon Auth](/docs/auth/overview) settings (e.g., OAuth or SMTP).
...
The Drizzle Studio integration that powers the [**Tables**](/docs/guides/tables) page in the Neon Console has been updated to version 1.3.2. This release fixes a regression where **Add Column** → **Review and Commit** could produce a zod validation error. For the latest improvements and fixes, see the [Neon Drizzle Studio Integration Changelog](https://github.com/neondatabase/neon-drizzle-studio-changelog/blob/main/CHANGELOG.md).
...
On paid plans, the snapshot limit (10) now applies only to manual snapshots. Scheduled backup snapshots no longer count toward that limit, so scheduled backups will no longer fail when you've already created your maximum number of manual snapshots. For more information, see [Backup & restore](/docs/guides/backup-restore).
