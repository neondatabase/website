...
- Added API methods for Neon's [project sharing](/docs/guides/project-sharing-guide) feature. The new methods allow you to:
  - List users that a project is shared with. See [Return project permissions](/docs/reference/api/projects/list-project-permissions).
  - Share a project with a specified user. See [Grant project permissions](/docs/reference/api/projects/grant-permission-to-project).
  - Revoke project access from a user you shared a project with. See [Revoke project permissions](/docs/reference/api/projects/revoke-permission-from-project).
- Neon API methods such as [Get projects](/docs/reference/api/projects/list-projects), which return the creation source in the response body, now return `"creation_source": "vercel"` for projects and branches created by the [Neon Vercel Integration](https://vercel.com/integrations/neon).
- The `ips` field in the `allowed_ips` object in the [Create project](/docs/reference/api/projects/create-project) and [Update project](/docs/reference/api/projects/update-project) APIs was changed from required to optional. This change allows certain IP Allow requests to be sent from the [interactive Neon API Reference](/docs/reference/api) without specifying IP addresses.

### Neon CLI updates

The [Neon CLI](/docs/cli) now includes an `ip-allow` command, which supports `list`, `add`, `remove`, and `reset` actions on the IP allowlist for your Neon project. For details, see [Neon CLI commands — ip-allow](/docs/cli/ip-allow). To install the Neon CLI or update your installation to the latest version, see [Neon CLI — Install and connect](/docs/cli/install).
...
- The **Community** link in the Neon Console sidebar has been updated to direct users to the [Neon Discord](https://discord.gg/N8kvBm7qH5) server, rather than the [Neon Discourse forum](https://community.neon.tech/).
