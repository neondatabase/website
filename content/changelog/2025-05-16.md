...
The Data API is in Early Access and requires an invite. Message us from the [Console](https://console.neon.tech/app/projects?modal=feedback) or on [Discord](https://discord.gg/N8kvBm7qH5) and we'll get you set up.
...
- The [Retrieve project consumption metrics](/docs/reference/api/consumption/get-consumption-history-per-project) API now returns a `logical_size_bytes_hour` value, which is the logical data size consumed on an hourly basis.
