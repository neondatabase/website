...
For API details, see [Retrieve project consumption metrics](/docs/reference/api/consumption/get-consumption-history-per-project-v2). For more information, see [Querying consumption metrics](/docs/guides/consumption-metrics).
...
- You can now set your project's default branch using the [Update Project API](/docs/reference/api/projects/update-project) by passing the `default_branch_id` parameter. This makes it easier to automate branch management in CI/CD pipelines and scripts, for example, after a recovery operation or when promoting a development branch to production. The previous default branch is automatically unset.
