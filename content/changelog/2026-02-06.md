...
The quickest way to set up the Neon MCP Server is:

```bash
npx neonctl@latest init
```

Use this if you want the MCP server together with our agent skills and VS Code extension—it configures the MCP server for all detected AI agents and editors in your workspace (Cursor, VS Code, Claude Code, and others) using API key authentication. If you only want the MCP server and nothing else, use `npx add-mcp https://mcp.neon.tech/mcp` instead; it connects to Neon's remote MCP server and an OAuth window will open in your browser when you use the connection to authorize access.
