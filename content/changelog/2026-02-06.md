---
title: Simpler MCP Server setup, track usage programmatically, and new PG18 extension support
---
...
This command auto-detects compatible MCP clients (like Cursor, VS Code, Claude Code, and others) and configures them to connect to Neon's remote MCP server. An OAuth window will open in your browser to authorize access to your Neon account.

With OAuth authentication, the MCP server operates on projects under your personal Neon account by default. To access projects under an organization, provide either the `org_id` or `project_id` in your prompt.

For API key-based authentication (useful for remote agents or organizations), use:

```bash
npx add-mcp https://mcp.neon.tech/mcp --header "Authorization: Bearer $NEON_API_KEY"
```

For more setup options, see [Connect MCP clients to Neon](/docs/ai/connect-mcp-clients-to-neon).
