...
- **pg_search version update**
  - We updated the `pg_search` extension from version **0.15.18** to **0.15.26**. If you are a current users of the `pg_search` extension, update your installation to the latest version by running `ALTER EXTENSION pg_search UPDATE`. Failign to do so could result in `pg_search` workload issues.
