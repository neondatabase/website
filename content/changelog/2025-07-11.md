...
You can now delete Neon Auth users directly from the Auth UI (or programmatically via the [Delete Auth User](/docs/reference/api/auth-legacy/delete-neon-auth-user)) API endpoint. This action soft-deletes the user in `neon_auth.users_sync` by setting the `deleted_at` column, rather than removing the record entirely.
