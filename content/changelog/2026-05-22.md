...
For the full integration guide, see [AI agent integration](/docs/guides/ai-agent-integration). To learn more about building agent platforms with Neon and how this skill helps, see the [blog post](https://neon.com/blog/neon-for-agent-platforms).

## Neon MCP Server: branch from any parent branch

The [`create_branch`](/docs/ai/neon-mcp-server) tool now accepts an optional `parentId` so you can fork any existing branch, not only the project default. If you omit `parentId`, behavior is unchanged.
...
Your agent can call `create_branch` with `parentId` set to that branch ID. See the [supported MCP tools](/docs/ai/neon-mcp-server) list for parameters.
