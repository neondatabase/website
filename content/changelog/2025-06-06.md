...
Get started immediately by visiting [pg.new](https://pg.new) in your browser, running `npx get-db` from the command line, or integrating automatic database provisioning into Vite projects with `vite-plugin-db`.

**Update:** At the time of this release it was at neon.new; it's now at pg.new. The CLI was `npx neondb` and the Vite plugin was `@neondatabase/vite-plugin-postgres`; they are now `npx get-db` and `vite-plugin-db`.
