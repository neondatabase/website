...
Three new services are coming to Neon: **Storage**, **Compute**, and an **AI Gateway**, all built around the same instant, branchable, serverless properties as Postgres. Together with **Postgres**, **Neon Auth**, and the **Data API**, Neon becomes a complete backend for apps and agents.
...
- **AI Gateway:** An LLM gateway for AI workloads, integrated with Neon Auth. Covers model routing, logging, and cost controls, built on the same infrastructure that handles 125 trillion tokens per month on Databricks. [Sign up for early access](https://console.neon.tech/app/settings/early-access) and read the [full announcement](https://neon.com/blog/were-building-backends).
