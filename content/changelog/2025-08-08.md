...
## Snapshot scheduling and more restore options
...
To give your branch an end date, set the `expires_at` timestamp (RFC 3339) when creating or updating the branch via the API, CLI, or Neon Console, up to 30 days ahead of time. When that time comes, the branch and its compute endpoints are permanently deleted.
