...
- The [neon_superuser](/docs/manage/roles#the-neonsuperuser-role) role now has membership in the predefined `pg_monitor` Postgres role, which provides read/execute privileges on various Postgres monitoring views and functions. For example, with membership in the `pg_monitor` role, the [neon_superuser] role can now:
    ...
    The `neon_superuser` role also has `WITH ADMIN` on the `pg_monitor` role, which enables granting the `pg_monitor` to other Postgres roles.
...
**Reminder**: For ongoing discussions, support, and community engagement, please join us on our [Discord](https://discord.com/invite/92vNTzKDGp) server.
