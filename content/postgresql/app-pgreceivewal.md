...
* `-S slotname``--slot=slotname`
  ...
  Require pg\_receivewal to use an existing replication slot (see [Section 27.2.6](warm-standby#STREAMING-REPLICATION-SLOTS)). When this option is used, pg\_receivewal will report a flush position to the server, indicating when each segment has been synchronized to disk so that the server can remove that segment if it is not otherwise needed.
...
[pg\_basebackup](app-pgbasebackup)