...
This tutorial picks up from where the [Updating Data in a Table Tutorial](/postgresql/postgresql-python/update) left off.
...
When you send the first SQL statement to the PostgreSQL database using a `cursor` object, `psycopg2` initiates a new [transaction](/postgresql/postgresql-transaction).
...
The `autocommit` mode can be particularly useful when executing statements that need to operate outside a transaction, such as [CREATE DATABASE](/postgresql/postgresql-administration/postgresql-create-database) and `VACUUM`.
...
- First, [insert a new row](/postgresql/postgresql-insert) into the `parts` table and get the part id.
-
