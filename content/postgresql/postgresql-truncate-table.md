...
To remove all data from a table, you use the `DELETE` statement without a [WHERE](/postgresql/postgresql-where) clause. However, when the table has numerous data, the `DELETE` statement is not efficient. In this case, you can use the `TRUNCATE TABLE` statement.
...
In practice, the table you want to delete all data often has [foreign key](/postgresql/postgresql-foreign-key) references from other tables.
...
Besides removing data, you may want to reset the values of the [identity column](/postgresql/postgresql-identity-column) by using the `RESTART IDENTITY` option like this:
...
For example, the following statement removes all rows from the `products` table and resets the [sequence](/postgresql/postgresql-sequences) associated with the `id` column:
...
Even though the `TRUNCATE TABLE` statement removes all data from a table, it does not fire any `ON DELETE` [triggers](/postgresql/postgresql-triggers) associated with the table.
