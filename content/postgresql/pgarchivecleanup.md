...
pg\_archivecleanup is designed to be used as an `archive_cleanup_command` to clean up WAL file archives when running as a standby server (see [Section 27.2](warm-standby)). pg\_archivecleanup can also be used as a standalone program to clean WAL file archives.
