...
We use the [CREATE TABLE statement](/postgresql/postgresql-create-table) to create the `books` table as follows:
...
PostgreSQL provides the `hstore_to_json()` function to convert hstore data to [JSON](/postgresql/postgresql-json). See the following statement:
