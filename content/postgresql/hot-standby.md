...
When a conflicting query is short, it's typically desirable to allow it to complete by delaying WAL application for a little bit; but a long delay in WAL application is usually not desirable. So the cancel mechanism has parameters, [max\_standby\_archive\_delay](runtime-config-replication#GUC-MAX-STANDBY-ARCHIVE-DELAY) and [max\_standby\_streaming\_delay](runtime-config-replication#GUC-MAX-STANDBY-STREAMING-DELAY), that define the maximum allowed delay in WAL application. Conflicting queries will be canceled once it has taken longer than the relevant delay setting to apply any newly-received WAL data. There are two parameters so that different delay values can be specified for the case of reading WAL data from an archive (i.e., initial recovery from a base backup or “catching up” a standby server that has fallen far behind) versus reading WAL data via streaming replication.
...
Row version cleanup isn't the only potential cause of conflicts with standby queries. All index-only scans (including those that run on standbys) must use an MVCC snapshot that “agrees” with the visibility map. Conflicts are therefore required whenever `VACUUM` [sets a page as all-visible in the visibility map](routine-vacuuming#VACUUM-FOR-VISIBILITY-MAP "25.1.4. Updating the Visibility Map") containing one or more rows *not* visible to all standby queries. So even running `VACUUM` against a table with no updated or deleted rows requiring cleanup might lead to conflicts.
...
Users can control whether a log message is produced when WAL replay is waiting longer than `deadlock_timeout` for conflicts. This is controlled by the [log\_recovery\_conflict\_waits](runtime-config-logging#GUC-LOG-RECOVERY-CONFLICT-WAITS) parameter.
...
It is important that the administrator select appropriate settings for [max\_standby\_archive\_delay](runtime-config-replication#GUC-MAX-STANDBY-ARCHIVE-DELAY) and [max\_standby\_streaming\_delay](runtime-config-replication#GUC-MAX-STANDBY-STREAMING-DELAY). The best choices vary depending on business priorities. For example if the server is primarily tasked as a High Availability server, then you will want low delay settings, perhaps even zero, though that is a very aggressive setting. If the standby server is tasked as an additional server for decision support queries then it might be acceptable to set the maximum delay values to many hours, or even -1 which means wait forever for queries to complete.
...
Various parameters have been mentioned above in [Section 27.4.2](hot-standby#HOT-STANDBY-CONFLICT "27.4.2. Handling Query Conflicts") and [Section 27.4.3](hot-standby#HOT-STANDBY-ADMIN "27.4.3. Administrator's Overview").

On the primary, the [wal\_level](runtime-config-wal#GUC-WAL-LEVEL) parameter can be used. [max\_standby\_archive\_delay](runtime-config-replication#GUC-MAX-STANDBY-ARCHIVE-DELAY) and [max\_standby\_streaming\_delay](runtime-config-replication#GUC-MAX-STANDBY-STREAMING-DELAY) have no effect if set on the primary.

On the standby, parameters [hot\_standby](runtime-config-replication#GUC-HOT-STANDBY), [max\_standby\_archive\_delay](runtime-config-replication#GUC-MAX-STANDBY-ARCHIVE-DELAY) and [max\_standby\_streaming\_delay](runtime-config-replication#GUC-MAX-STANDBY-STREAMING-DELAY) can be used.
