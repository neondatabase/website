...
|                                                                                                |                                                                                                        |                                                                                                    |                                                                                                                     |
| ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| [btree\_gin](btree-gin "F.8. btree_gin — GIN operator classes with B-tree behavior")      | [fuzzystrmatch](fuzzystrmatch "F.17. fuzzystrmatch — determine string similarities and distance") | [ltree](ltree "F.23. ltree — hierarchical tree-like data type")                               | [tcn](tcn "F.43. tcn — a trigger function to notify listeners of changes to table content")                    |
| [btree\_gist](btree-gist "F.9. btree_gist — GiST operator classes with B-tree behavior")  | [hstore](hstore "F.18. hstore — hstore key/value datatype")                                       | [pgcrypto](pgcrypto "F.27. pgcrypto — cryptographic functions")                               | [tsm\_system\_rows](tsm-system-rows "F.45. tsm_system_rows — the SYSTEM_ROWS sampling method for TABLESAMPLE") |
| [citext](citext "F.10. citext — a case-insensitive character string type")                | [intarray](intarray "F.20. intarray — manipulate arrays of integers")                             | [pg\_trgm](pgtrgm "F.34. pg_trgm — support for similarity of text using trigram matching")    | [tsm\_system\_time](tsm-system-time "F.46. tsm_system_time — the SYSTEM_TIME sampling method for TABLESAMPLE") |
...
| [dict\_int](dict-int "F.13. dict_int — example full-text search dictionary for integers") | [lo](lo "F.22. lo — manage large objects")                                                        | [tablefunc](tablefunc "F.42. tablefunc — functions that return tables (crosstab and others)") | [uuid-ossp](uuid-ossp "F.48. uuid-ossp — a UUID generator")                                                    |
...
Note, however, that some of these components are not “extensions” in this sense, but are loaded into the server in some other way, for instance by way of [shared\_preload\_libraries](runtime-config-client#GUC-SHARED-PRELOAD-LIBRARIES). See the documentation of each component for details.