...
- [F.26. pg_buffercache — inspect PostgreSQL buffer cache state](pgbuffercache)
...
- [F.28. pg_freespacemap — examine the free space map](pgfreespacemap)
...
- [F.29. pg_prewarm — preload relation data into buffer caches](pgprewarm)
...
- [F.31. pg_stat_statements — track statistics of SQL planning and execution](pgstatstatements)
...
- [F.33. pg_surgery — perform low-level surgery on relation data](pgsurgery)

  - [F.33.1. Functions](pgsurgery#PGSURGERY-FUNCS)
  - [F.33.2. Authors](pgsurgery#PGSURGERY-AUTHORS)

- [F.34. pg_trgm — support for similarity of text using trigram matching](pgtrgm)
...
- [F.35. pg_visibility — visibility map information and utilities](pgvisibility)

  - [F.35.1. Functions](pgvisibility#PGVISIBILITY-FUNCS)
  - [F.35.2. Author](pgvisibility#PGVISIBILITY-AUTHOR)

- [F.36. pg_walinspect — low-level WAL inspection](pgwalinspect)

  - [F.36.1. General Functions](pgwalinspect#PGWALINSPECT-FUNCS)
  - [F.36.2. Author](pgwalinspect#PGWALINSPECT-AUTHOR)

- [F.37. postgres_fdw — access data stored in external PostgreSQL servers](postgres-fdw)

  - [F.37.1. FDW Options of postgres_fdw](postgres-fdw#POSTGRES-FDW-OPTIONS)
...
- [F.39. sepgsql — SELinux-, label-based mandatory access control (MAC) security module](sepgsql)
  ...
  - [F.39.5. Features](sepgsql#SEPGSQL-FEATURES)
  - [F.39.6. Sepgsql Functions](sepgsql#SEPGSQL-FUNCTIONS)
  - [F.39.7. Limitations](sepgsql#SEPGSQL-LIMITATIONS)
  - [F.39.8. External Resources](sepgsql#SEPGSQL-RESOURCES)
  - [F.39.9. Author](sepgsql#SEPGSQL-AUTHOR)
...
- [F.40. spi — Server Programming Interface features/examples](contrib-spi)
  ...
  - [F.40.3. insert_username — Functions for Tracking Who Changed a Table](contrib-spi#CONTRIB-SPI-INSERT-USERNAME)
...
- [F.44. test_decoding — SQL-based test/example module for WAL logical decoding](test-decoding)
- [F.45. tsm_system_rows — the `SYSTEM_ROWS` sampling method for `TABLESAMPLE`](tsm-system-rows)

  - [F.45.1. Examples](tsm-system-rows#TSM-SYSTEM-ROWS-EXAMPLES)

- [F.46. tsm_system_time — the `SYSTEM_TIME` sampling method for `TABLESAMPLE`](tsm-system-time)
...
This appendix covers extensions and other server plug-in module libraries found in `contrib`. [Appendix G](contrib-prog 'Appendix G. Additional Supplied Programs') covers utility programs.

When building from the source distribution, these optional components are not built automatically, unless you build the "world" target (see [Step 2](install-make#BUILD 'Build')). You can build and install all of them by running:
...
|                                                                                          |                                                                                                   |                                                                                               |                                                                                                              |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| [btree_gin](btree-gin 'F.8. btree_gin — GIN operator classes with B-tree behavior')      | [fuzzystrmatch](fuzzystrmatch 'F.17. fuzzystrmatch — determine string similarities and distance') | [ltree](ltree 'F.23. ltree — hierarchical tree-like data type')                               | [tcn](tcn 'F.43. tcn — a trigger function to notify listeners of changes to table content')                  |
| [btree_gist](btree-gist 'F.9. btree_gist — GiST operator classes with B-tree behavior')  | [hstore](hstore 'F.18. hstore — hstore key/value datatype')                                       | [pgcrypto](pgcrypto 'F.27. pgcrypto — cryptographic functions')                               | [tsm_system_rows](tsm-system-rows 'F.45. tsm_system_rows — the SYSTEM_ROWS sampling method for TABLESAMPLE') |
| [citext](citext 'F.10. citext — a case-insensitive character string type')               | [intarray](intarray 'F.20. intarray — manipulate arrays of integers')                             | [pg_trgm](pgtrgm 'F.34. pg_trgm — support for similarity of text using trigram matching')     | [tsm_system_time](tsm-system-time 'F.46. tsm_system_time — the SYSTEM_TIME sampling method for TABLESAMPLE') |
| [cube](cube 'F.11. cube — a multi-dimensional cube data type')                           | [isn](isn 'F.21. isn — data types for international standard numbers (ISBN, EAN, UPC, etc.)')     | [seg](seg 'F.38. seg — a datatype for line segments or floating point intervals')             | [unaccent](unaccent 'F.47. unaccent — a text search dictionary which removes diacritics')                    |
| [dict_int](dict-int 'F.13. dict_int — example full-text search dictionary for integers') | [lo](lo 'F.22. lo — manage large objects')                                                        | [tablefunc](tablefunc 'F.42. tablefunc — functions that return tables (crosstab and others)') | [uuid-ossp](uuid-ossp 'F.48. uuid-ossp — a UUID generator')                                                  |
