...
In PostgreSQL, a view is a named query stored in the PostgreSQL database server. A simple [view can be updatable](/postgresql/postgresql-views/postgresql-updatable-views).
...
To change the scope of check for an existing view, you can use the [ALTER VIEW](/postgresql/postgresql-views/postgresql-alter-view) statement.
...
The following statements [create a new table](/postgresql/postgresql-create-table) called `employees` and [insert data into it](/postgresql/postgresql-tutorial/postgresql-insert-multiple-rows):
...
After adding the `WITH CHECK OPTION`, you perform insert, [update](/postgresql/postgresql-update), and [delete](/postgresql/postgresql-tutorial/postgresql-delete) on `employees` table that satisfies the `WHERE` clause in the defining query of the view.
