---
prevPost: /postgresql/postgresql-isnull
nextPost: /postgresql/postgresql-cast-convert-a-value-of-one-type-to-another
...
First, [create a table](/postgresql/postgresql-create-table) called `posts`:
...
Second, [insert some sample data](/postgresql/postgresql-insert) into the `posts` table.
...
Third, use the [COALESCE function](/postgresql/postgresql-coalesce) to handle `NULL` in the `body` column:
...
First, [create a new table](/postgresql/postgresql-create-table) named members:
...
Second, [insert some rows](/postgresql/postgresql-insert) for testing:
...
In this example, we use the [SUM function](/postgresql/postgresql-aggregate-functions/postgresql-sum-function) and [CASE expression](/postgresql/postgresql-case) to calculate the total number of male members. Then we divide the total of male members by the total of female members to get the ratio. In this case, it returns 200%:
