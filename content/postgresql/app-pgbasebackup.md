...
Whenever pg\_basebackup is taking a base backup, the server's `pg_stat_progress_basebackup` view will report the progress of the backup. See [Section 28.4.6](progress-reporting#BASEBACKUP-PROGRESS-REPORTING) for details.
...
* `--manifest-checksums=algorithm`
  ...
  [pg\_verifybackup](app-pgverifybackup) can be used to check the integrity of a backup against the backup manifest.
...
* `--no-manifest`
  ...
  Disables generation of a backup manifest. If this option is not specified, the server will generate and send a backup manifest which can be verified using [pg\_verifybackup](app-pgverifybackup). The manifest is a list of every file present in the backup with the exception of any WAL files that may be included. It also stores the size, last modification time, and an optional checksum for each file.
...
[pg\_dump](app-pgdump), [Section 28.4.6](progress-reporting#BASEBACKUP-PROGRESS-REPORTING)