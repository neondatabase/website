...
To make the output more readable, you can use the [cast](https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-cast/) operator as follows:
...
In the query, we joined the `payment` table with the `customer` table using [inner join](https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-inner-join/). We used `GROUP BY` clause to group customers into groups and applied the `AVG()` function to calculate the average per group.
...
First, [create a table](https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-create-table/) named `t1`.
...
Second, [insert](https://www.postgresqltutorial.com/postgresql-tutorial/postgresql-insert/) some sample data:
