...
For convenience, the programs [createuser](app-createuser "createuser") and [dropuser](app-dropuser "dropuser") are provided as wrappers around these SQL commands that can be called from the shell command line:
...
The [psql](app-psql "psql") program's `\du` meta-command is also useful for listing the existing roles.
