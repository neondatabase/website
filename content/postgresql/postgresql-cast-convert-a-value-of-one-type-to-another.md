...
There are many cases in which you want to convert a value of one [type](/postgresql/postgresql-data-types) into another. PostgreSQL offers the `CAST()` function and cast operator (`::`) to do this.
...
- Then, specify the target [data type](/postgresql/postgresql-data-types) to which you want to convert the `value`.
...
This example uses the `CAST()` function to convert a string to a [date](/postgresql/postgresql-date):
...
This example uses the cast operator (::) to convert a string to a [timestamp](/postgresql/postgresql-timestamp):
...
This example uses the cast operator to convert a string to an [interval](/postgresql/postgresql-interval):
...
First, [create](/postgresql/postgresql-create-table) a `ratings` table that consists of two columns: `id`and `rating`:
...
Second, [insert some sample data](/postgresql/postgresql-insert) into the `ratings` table.
