...
PostgreSQL has the ability to report the progress of certain commands during command execution. Currently, the only commands which support progress reporting are `ANALYZE`, `CLUSTER`, `CREATE INDEX`, `VACUUM`, `COPY`, and [BASE\_BACKUP](protocol-replication#PROTOCOL-REPLICATION-BASE-BACKUP) (i.e., replication command that [pg\_basebackup](app-pgbasebackup "pg_basebackup") issues to take a base backup). This may be expanded in the future.
...
| `phase` `text`Current processing phase. See [Table 28.38](progress-reporting#ANALYZE-PHASES "Table 28.38. ANALYZE Phases").                               |
...
| `phase` `text`Current processing phase. See [Table 28.40](progress-reporting#CLUSTER-PHASES "Table 28.40. CLUSTER and VACUUM FULL Phases").                            |
...
| `phase` `text`Current processing phase of index creation. See [Table 28.43](progress-reporting#CREATE-INDEX-PHASES "Table 28.43. CREATE INDEX Phases").                                                         |
...
| `phase` `text`Current processing phase of vacuum. See [Table 28.45](progress-reporting#VACUUM-PHASES "Table 28.45. VACUUM Phases").                                                                                                                                                                                                                                                                               |
...
| `heap_blks_scanned` `bigint`Number of heap blocks scanned. Because the [visibility map](storage-vm "73.4. Visibility Map") is used to optimize scans, some blocks will be skipped without inspection; skipped blocks are included in this total, so that this number will eventually become equal to `heap_blks_total` when the vacuum is complete. This counter only advances when the phase is `scanning heap`. |
...
| `vacuuming indexes`        | `VACUUM` is currently vacuuming the indexes. If a table has any indexes, this will happen at least once per vacuum, after the heap has been completely scanned. It may happen multiple times per vacuum if [maintenance\_work\_mem](runtime-config-resource#GUC-MAINTENANCE-WORK-MEM) (or, in the case of autovacuum, [autovacuum\_work\_mem](runtime-config-resource#GUC-AUTOVACUUM-WORK-MEM) if set) is insufficient to store the number of dead tuples found. |
...
| `phase` `text`Current processing phase. See [Table 28.47](progress-reporting#BASEBACKUP-PHASES "Table 28.47. Base Backup Phases").                                                                                                                                                                                                                                                                                                                                                                                                                           |
