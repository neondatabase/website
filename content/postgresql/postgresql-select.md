...
redirectFrom:
   - /postgresql/postgresql-tutorial/postgresql-select
...
Note that if you don't know how to execute a query against the PostgreSQL database using the **psql** command-line tool or **pgAdmin** GUI tool, you can check [the connection to the PostgreSQL database tutorial](/postgresql/postgresql-getting-started/connect-to-postgresql-database).
...
-
- Join with other tables using [joins](/postgresql/postgresql-joins) such as `INNER JOIN`, `LEFT JOIN`, `FULL OUTER JOIN`, `CROSS JOIN` clauses.
-
...
We will use the following `customer` table in the `dvdrental` [sample\*\* \*\*database](/postgresql/postgresql-getting-started/postgresql-sample-database) for the demonstration.

![customer table](/postgresqltutorial_data/wp-content-uploads-2019-05-customer.png)

First, [connect to the PostgreSQL server](/postgresql/postgresql-getting-started/connect-to-postgresql-database) using the `postgres` user:
...
However, using the asterisk (`*`) in the `SELECT` statement is considered a bad practice when you embed SQL statements in the application code, such as [Python](/postgresql/postgresql-python), [Java](/postgresql/postgresql-jdbc), or [PHP](/postgresql/postgresql-php) for the following reasons:
...
In this example, we used the [concatenation operator](/postgresql/postgresql-string-functions/postgresql-concat-function) `||` to concatenate the first name, space, and last name of every customer.

Notice the first column of the output doesn't have a name but `?column?`. To assign a name to a column temporarily in the query, you can use a [column alias](/postgresql/postgresql-column-alias):
