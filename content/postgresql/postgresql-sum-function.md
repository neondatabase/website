...
redirectFrom:
            ...
            - /postgresql/postgresql-aggregate-functions/postgresql-sum-function
...
The PostgreSQL `SUM()` is an [aggregate function](/postgresql/postgresql-aggregate-functions) that returns the sum of values in a set.
...
Let's take some examples of using the `SUM()` function. We'll use the `payment` table in the [sample database](/postgresql/postgresql-getting-started/postgresql-sample-database).
