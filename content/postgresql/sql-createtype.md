...
The second form of `CREATE TYPE` creates an enumerated (enum) type, as described in [Section 8.7](datatype-enum "8.7. Enumerated Types"). Enum types take a list of quoted labels, each of which must be less than `NAMEDATALEN` bytes long (64 bytes in a standard PostgreSQL build). (It is possible to create an enumerated type with zero labels, but such a type cannot be used to hold values before at least one label is added using [`ALTER TYPE`](sql-altertype "ALTER TYPE").)

### Range Types

The third form of `CREATE TYPE` creates a new range type, as described in [Section 8.17](rangetypes "8.17. Range Types").
...
To indicate that a type is a fixed-length array type, specify the type of the array elements using the `ELEMENT` key word. For example, to define an array of 4-byte integers (`int4`), specify `ELEMENT = int4`. For more details, see [Array Types](sql-createtype#SQL-CREATETYPE-ARRAY "Array Types") below.
...
* *`category`*
    ...
    The category code (a single ASCII character) for this type. The default is `'U'` for “user-defined type”. Other standard category codes can be found in [Table 53.65](catalog-pg-type#CATALOG-TYPCATEGORY-TABLE "Table 53.65. typcategory Codes"). You may also choose other ASCII characters in order to create custom categories.
...
More examples, including suitable input and output functions, are in [Section 38.13](xtypes "38.13. User-Defined Types").
...
[ALTER TYPE](sql-altertype "ALTER TYPE"), [CREATE DOMAIN](sql-createdomain "CREATE DOMAIN"), [CREATE FUNCTION](sql-createfunction "CREATE FUNCTION"), [DROP TYPE](sql-droptype "DROP TYPE")