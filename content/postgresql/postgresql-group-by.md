...
For each group, you can apply an [aggregate function](/postgresql/postgresql-aggregate-functions) such as `SUM()` to calculate the sum of items or `COUNT()` to get the number of items in the groups.
...
PostgreSQL evaluates the `GROUP BY` clause after the `FROM` and `WHERE` clauses and before the `HAVING` `SELECT`, `DISTINCT`, [`ORDER BY`](/postgresql/postgresql-order-by)and `LIMIT` clauses.
...
The `GROUP BY` clause is useful when used in conjunction with an [aggregate function](/postgresql/postgresql-aggregate-functions).
...
In this example, we join the `payment` table with the `customer` table using an [inner join](/postgresql/postgresql-inner-join) to get the customer names and group customers by their names.
...
Since the values in the `payment_date` column are [timestamps](/postgresql/postgresql-timestamp), we cast them to date values using the [cast operator](/postgresql/postgresql-tutorial/postgresql-cast) `::`.
