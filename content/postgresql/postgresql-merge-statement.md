...
-
- `source_query` is a source table or a [SELECT](/postgresql/postgresql-select) statement that provides the data for the merge operation.
-
...
First, [create two tables](/postgresql/postgresql-create-table) called `leads` and `customers`:
...
First, [insert two rows](/postgresql/postgresql-insert-multiple-rows) into the `leads` table:
...
First, [insert a new row](/postgresql/postgresql-insert) into the `leads` table and [update](/postgresql/postgresql-tutorial/postgresql-update) the `name` of the row with id 2:
