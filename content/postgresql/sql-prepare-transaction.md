...
Once prepared, a transaction can later be committed or rolled back with [`COMMIT PREPARED`](sql-commit-prepared "COMMIT PREPARED") or [`ROLLBACK PREPARED`](sql-rollback-prepared "ROLLBACK PREPARED"), respectively. Those commands can be issued from any session, not only the one that executed the original transaction.
...
This command must be used inside a transaction block. Use [`BEGIN`](sql-begin "BEGIN") to start one.
...
[COMMIT PREPARED](sql-commit-prepared "COMMIT PREPARED"), [ROLLBACK PREPARED](sql-rollback-prepared "ROLLBACK PREPARED")