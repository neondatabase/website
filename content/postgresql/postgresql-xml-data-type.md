...
- `id` is an [identity column](/postgresql/postgresql-identity-column) that serves as the [primary key](/postgresql/postgresql-tutorial/postgresql-primary-key) column of the table.
-
- `info` is a column with the type XML that will store the XML data.

Second, [insert a row](/postgresql/postgresql-insert) into the `person` table:
...
Third, [insert multiple rows](/postgresql/postgresql-insert-multiple-rows) into the `person` table:
...
First, create an [index expression](/postgresql/postgresql-indexes/postgresql-index-on-expression) that extracts the name of a person as an array of text:
