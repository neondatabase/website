...
It creates a replication-mode connection, so it is subject to the same constraints as [pg\_receivewal](app-pgreceivewal), plus those for logical replication (see [Chapter 49](logicaldecoding)).
...
* `-s interval_seconds``--status-interval=interval_seconds`
  ...
  This option has the same effect as the option of the same name in [pg\_receivewal](app-pgreceivewal). See the description there.
...
* `-d dbname``--dbname=dbname`
  ...
  The database to connect to. See the description of the actions for what this means in detail. The *`dbname`* can be a [connection string](libpq-connect#LIBPQ-CONNSTRING). If so, connection string parameters will override any conflicting command line options. Defaults to the user name.
...
[pg\_receivewal](app-pgreceivewal)