...
In PostgreSQL, an enum type is a [custom data type](/postgresql/postgresql-user-defined-data-types) that allows you to define a list of possible values for a column.
...
If you attempt to [insert](/postgresql/postgresql-insert) or [update](/postgresql/postgresql-tutorial/postgresql-update) a row with a value not in the list, PostgreSQL will issue an error.
...
Second, [create a table](/postgresql/postgresql-create-table) called `requests` that has a column using `priority` enum:
...
Third, [insert some rows](/postgresql/postgresql-insert) into the `requests` table:
...
There is some similarity between enums and [foreign keys](/postgresql/postgresql-foreign-key). Both allow you to define a set of values for a column.
