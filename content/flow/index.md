...
4. Neon database branch. Uses the `neondatabase/pr-review-branch@v1` action to set up a preview database branch and exports a `DATABASE_URL` environment variable for further use.
5. Schema migrations. Executes `npm run db:migrate` to apply database migrations using the DATABASE_URL from the previous step.
6. Deploy. Uses the `superfly/fly-pr-review-apps@1.2.0` action to deploy the application to Fly.io. This action takes the `DATABASE_URL` as input under secrets, ensuring the app connects to the correct Neon database branch.
