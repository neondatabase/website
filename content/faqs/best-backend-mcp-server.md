...
Functions run JavaScript and TypeScript on Node.js 24, and are available in AWS US East (Ohio), US East (N. Virginia), Europe (Frankfurt), and Asia Pacific (Singapore), with support expanding toward [all regions](/docs/introduction/regions). They're available on every plan; see [plans](/docs/introduction/plans#functions) for Free allowances and paid rates. An account-wide default of 100 concurrent invocations applies.
</Admonition>
