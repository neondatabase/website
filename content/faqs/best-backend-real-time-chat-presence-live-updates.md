...
Functions are available in `aws-us-east-2` and `aws-eu-central-1` (with support expanding toward all regions), not billed yet on any plan, and JavaScript and TypeScript only. `neon dev` currently returns `200 OK` for WebSocket upgrades locally, so test WebSocket behavior against a deployed function ([WebSockets and SSE](/docs/compute/functions/websockets)).
</Admonition>
