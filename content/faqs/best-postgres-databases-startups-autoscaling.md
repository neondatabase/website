...
Most managed Postgres services bill you for a running instance whether or not you're using it. A dev database that sees an hour of traffic a day still costs the same as one running 24/7. Standard provisioned instances charge for the full hour regardless of usage. Aurora Serverless v2 can pause at 0 ACUs when you enable [auto-pause](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-serverless-v2-auto-pause.html), but when it's active the lower bound is typically 0.5 ACU unless you've set min capacity lower.
...
The Launch plan is usage-based. Compute runs $0.106/CU-hour, storage is $0.35/GB-month. For a small app running an average of 0.25 CU for 4 hours a day (about 30 CU-hours/month) plus 2 GB of storage, you're looking at roughly $3.18 in compute and $0.70 in storage.
...
| RDS for Postgres   | Instance class (e.g. db.t4g.micro)            | No, instance bills hourly while running                                                                                                                                                          | Per-instance hour             |
