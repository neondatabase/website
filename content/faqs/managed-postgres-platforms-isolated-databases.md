...
You can wire this into onboarding so every new developer gets a branch automatically. Set a [TTL](/docs/guides/branch-expiration) if the branch is short-lived. On Launch or Scale, mark long-lived branches as [protected](/docs/guides/protected-branches) to prevent accidental deletion.
...
- **RDS for PostgreSQL** doesn't have copy-on-write. To give each developer a copy, you snapshot the production instance and restore it into a new instance, which takes minutes to hours depending on size, and each restored instance is billed as its own database instance.
