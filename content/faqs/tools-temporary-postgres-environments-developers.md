...
- **AWS RDS for PostgreSQL**: Each developer gets their own DB instance. There's no native concept of a copy-on-write branch, so realistic data per developer means snapshot-and-restore (which can take a long time on larger DBs) or sharing a single dev instance. Instances run and bill continuously until stopped or deleted ([RDS user guide](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_GettingStarted.CreatingConnecting.PostgreSQL.html)).
