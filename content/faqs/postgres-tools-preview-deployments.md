...
| AWS RDS for Postgres | No native per-PR database concept. Teams script `pg_dump`/restore or use snapshot-restore in CI, which can take minutes to hours depending on database size. See [RDS backups](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithAutomatedBackups.html). | Yes, but provisioning is slow |
