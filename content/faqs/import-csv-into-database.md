...
Create the target table in Neon, then run `\copy` from a `psql` session connected to your database. `\copy` streams the CSV from your local filesystem over the existing connection, so it doesn't need access to the server's filesystem. For larger or messier CSVs, `pgloader` handles encoding, type coercion, and parallel loading. See [Import data from CSV](/docs/import/import-from-csv).

## Import a CSV with psql

First, create the destination table. Unless you list columns in the `\copy` command, the table columns must match the CSV columns in number and order:
...
Then connect with `psql` and load the file. To get the connection command, click **Connect** in the Neon Console and pick `psql` from the drop-down menu.
...
If the CSV has fewer columns than the table, or a different column order, list the columns in CSV order:
...
`pgloader` is a third-party tool. Use it when:
...
You write a small config file that describes the source, target, and any transformations, and pgloader runs the load and reports rows that fail. See the [pgloader documentation](https://pgloader.readthedocs.io/) for syntax.
...
The server-side `COPY ... FROM '/path/to/file.csv'` form reads from the Postgres server's filesystem, which isn't accessible on Neon. `COPY ... FROM STDIN` works fine over a connection and is what `\copy` uses under the hood. Both psql `\copy` and `COPY FROM STDIN` from a driver stream the data from the client.
</Admonition>
