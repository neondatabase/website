...
- **Autoscaling.** Compute moves between a minimum and maximum CU based on load. Up to 2 CU on Free, up to 16 CU on Launch and Scale (Scale also supports fixed sizes up to 56 CU).
- **Scale to zero.** Compute suspends after 5 minutes of inactivity and resumes in a few hundred milliseconds on the next query. Free keeps the 5-minute timeout and can't disable it. Launch defaults to 5 minutes and can disable scale to zero. Scale is configurable from 1 minute to always-on. See [Scale to Zero](/docs/introduction/scale-to-zero).
...
- **Usage-based pricing.** $0.106/CU-hour on Launch, $0.35/GB-month for storage. No fixed monthly fee. When compute is suspended, CU-hours stop; storage continues to bill.
...
| Autoscaling        | Up to 2 CU (Free), 16 CU (Launch/Scale) | [Min/max ACU range](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/aurora-serverless-v2.how-it-works.html) per cluster | Manual resize, brief downtime | Manual via [compute add-ons](https://supabase.com/docs/guides/platform/compute-add-ons)                                                       |
