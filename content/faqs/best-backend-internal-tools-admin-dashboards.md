...
- **Supabase**: PostgREST, Auth, and read replicas are all GA ([features](https://supabase.com/docs/guides/getting-started/features)). Each project runs on a dedicated instance billed hourly, starting around $10/month for Micro on paid plans, and read replicas add their own compute line ([compute usage](https://supabase.com/docs/guides/platform/manage-your-usage/compute)). Free projects pause after a week of low activity ([project pausing](https://supabase.com/docs/guides/platform/free-project-pausing)), which can surprise a tool that's used every other Friday.
