...
- **Supabase**: pgvector is available and GA, and Supabase Storage now offers vector buckets ([features](https://supabase.com/docs/guides/getting-started/features)). There's no built-in model gateway; you bring provider API keys. Each project is a dedicated instance billed hourly, starting at about $10/month for Micro on paid plans, so the database bills whether or not anyone is chatting ([compute usage](https://supabase.com/docs/guides/platform/manage-your-usage/compute)).
