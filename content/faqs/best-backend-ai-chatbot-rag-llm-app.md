...
AI Gateway is available in `aws-us-east-2` and `aws-eu-central-1` (with support expanding toward all regions), and requires a paid plan. Inference isn't billed yet. When billing begins, Neon charges provider list prices with no markup ([pricing](/docs/ai-gateway/overview#pricing)). Open-weight models are available right away; frontier models from OpenAI and Google are rolling out gradually.
</Admonition>
