...
You can rename a role with `ALTER ROLE`, but the default role created with your project (typically `neondb_owner`) is managed by Neon and assumed by some Console flows and integrations. Renaming it can break the **Connect** widget, password reset, and the Neon-Managed Vercel integration. Create a new role instead if you need a different owner. See [Manage roles](/docs/manage/roles).
</Admonition>
