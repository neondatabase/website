...
Add `.env` to `.gitignore`. Keep the `sslmode=require` and `channel_binding=require` parameters intact: Neon requires TLS.
...
- **Pooled** (hostname has `-pooler`): for serverless functions, web apps, and high-concurrency clients. Goes through PgBouncer in transaction mode.
- **Direct**: for migrations, `pg_dump`, `LISTEN`/`NOTIFY`, logical replication, and any session-level features.
...
Hardcoding the URL in source code is a common source of credential leaks. Read it from `process.env.DATABASE_URL`, `os.environ`, or your secret manager. If you're deploying to Vercel, Render, Fly, or similar, set `DATABASE_URL` in the platform's env settings; the Neon-Vercel integration sets it automatically.
</Admonition>
