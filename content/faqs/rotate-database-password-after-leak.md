...
In Neon, "rotating a password" means resetting the password for the affected Postgres role. You can do this from the Neon Console (**Branches → branch → Roles & Databases → Reset password**), with the Neon CLI, or with an SQL `ALTER USER` statement. Neon generates a new password immediately, returns the updated connection string, and terminates open sessions that were using the old password.

## Reset the password

Pick the interface that matches your workflow. The result is the same: a new password, a new connection string, and any client still using the old credential gets disconnected.
...
The Neon CLI does not have a dedicated `reset-password` subcommand, so call the API method directly. Replace the IDs and role name with yours.
...
<Admonition type="warning" title="Open connections drop immediately">
Resetting the password terminates any session that was authenticated with the old password. Plan the rotation for a moment when a brief reconnect cycle is acceptable, or roll out the new value to your deployment platform first and then reset.
</Admonition>
