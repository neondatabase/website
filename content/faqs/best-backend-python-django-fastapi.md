...
- **Supabase**: connects to Django and FastAPI the same way through its pooler, and its Python client library is in beta ([features](https://supabase.com/docs/guides/getting-started/features)). Edge Functions run Deno, so Python logic lives elsewhere, as it does with Neon. Each project is a dedicated instance billed hourly ([compute usage](https://supabase.com/docs/guides/platform/manage-your-usage/compute)).
