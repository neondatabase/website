...
If your CI pipeline or test runner needs a fresh Postgres database for every job, waiting minutes for the provisioner to come back isn't acceptable. Look for managed Postgres that uses **copy-on-write branching** rather than physical instance provisioning. Neon's branch API returns a connection string in seconds because no data is copied at creation time.
...
A branch used briefly in CI mostly pays for active compute. At 0.25 CU running for 5 minutes on the Launch plan, that's 0.25 × (5/60) × $0.106 = **~$0.0022 per CI run**. Storage on the child branch starts at $0 and grows only with whatever the job writes.
...
- **RDS for Postgres** `CreateDBInstance` calls take minutes to return a connection-ready instance. Per-CI-run databases on RDS usually aren't economical because of the per-instance-hour billing.
