...
Neon also covers the other pieces a side project usually needs:
...
- **Files**: [Object Storage](/docs/storage/overview) gives the Free plan 5 GB per project.
- **A backend function**: [Neon Functions](/docs/compute/functions/overview) include 1 million invocations a month on the Free plan. Functions, Object Storage, and AI Gateway are available in AWS US East (Ohio), US East (N. Virginia), Europe (Frankfurt), and Asia Pacific (Singapore), with support expanding toward [all regions](/docs/introduction/regions).

<Admonition type="tip" title="The first request after idle">
Reactivation takes a few hundred milliseconds, so the first visitor after a quiet stretch waits slightly longer. Paid plans can disable scale to zero for a project that must answer instantly ([scale to zero](/docs/introduction/scale-to-zero)). A side project that can tolerate that delay can leave it on.
</Admonition>

## How other options compare

- **Supabase**: the Free plan pauses a project after a week of low activity, and a paused project serves nothing until you restore it from the dashboard ([project pausing](https://supabase.com/docs/guides/platform/free-project-pausing)). After a year paused, one-click restore closes and you download a backup instead ([upgrading](https://supabase.com/docs/guides/platform/upgrading#time-limits)). Free includes no backups ([backups](https://supabase.com/docs/guides/platform/backups)). To stop the pausing, you move to Pro at $25/month, where the project runs on an always-on Micro instance (about $10/month, covered by Pro's $10 in compute credits) whether or not anyone visits ([pricing](https://supabase.com/pricing), [compute usage](https://supabase.com/docs/guides/platform/manage-your-usage/compute)). The idle cost of one side project on Pro is $25/month; on Neon's Launch plan it's the storage bill ([Free plan comparison](/guides/neon-vs-supabase-free-plan#idle-behavior)).
- **Firebase**: no idle compute charge, with Firestore billed per document read and write beyond the daily free quotas ([pricing](https://firebase.google.com/pricing)). It's a NoSQL document database, so the difference is the data model rather than the bill.
