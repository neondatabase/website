...
Some things shouldn't run in a browser: a Stripe webhook, an email send, a call that needs a secret. Deploy that piece as a [Neon Function](/docs/compute/functions/overview) with `DATABASE_URL` injected and call it from the SPA. Functions are available in AWS US East (Ohio), US East (N. Virginia), Europe (Frankfurt), and Asia Pacific (Singapore), with support expanding toward [all regions](/docs/introduction/regions). Functions are in beta. Host the static bundle anywhere: Vercel, Netlify, Cloudflare Pages, or an S3 bucket.
