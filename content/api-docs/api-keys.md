...
You can also manage API keys from the CLI with [`neon api-keys`](/docs/cli/api-keys). The Neon CLI also supports OAuth-based authentication via [`neon login`](/docs/cli/login), which opens a browser to authorize access without requiring a manually created key.
