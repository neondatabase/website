...
The [HONC stack](https://honc.dev/) (Hono + ORM/Drizzle + Neon + Cloudflare) is a set of tools for building type-safe data APIs that run on Cloudflare's edge network.

This guide walks you through building a simple task management API with the HONC stack. You'll learn how to:

- Initialize a HONC project using `create-honc-app`.
- Define your database schema with Drizzle ORM.
- Use Neon as your Postgres database.
- Create API endpoints using the Hono framework.
- Run your application locally and deploy it to Cloudflare Workers.
- Test your API with the built-in Fiberplane API Playground.
...
Start a HONC project with the [`create-honc-app`](https://github.com/fiberplane/create-honc-app) CLI tool.
...
2.  **Apply migrations:**
    ...
    npm run db:migrate
    ```
    This applies the migrations to your Neon database. Your `tasks` table should now exist. You can verify this on the **Tables** page of your project in the Neon Console.
    ![Neon Console - Tasks table](/docs/guides/honc-neon-tasks-table.png)

## Adapt API endpoints for tasks

The `src/index.ts` file generated by `create-honc-app` contains Hono routes and Zod schemas for a sample `users` API. Adapt it into a REST API for `tasks` that supports standard HTTP methods (`GET`, `POST`, `PUT`, `DELETE`).
...
2.  **Modify Zod schemas:**
    First, use Zod to define the structure of task data for API requests and responses. Find the existing `UserSchema` and related definitions and replace them with schemas for `Task` (how a task looks when retrieved) and `NewTask` (how a new task looks when being created).
...
2.  **Test your API endpoints:**
    ...
    - **Create a task:**
      ...
      You can also check that the task was added by opening your project in the Neon Console. The task should appear in the `tasks` table.
...
To access it, start your local development server with `npm run dev`, then open [`localhost:8787/fp`](http://localhost:8787/fp).

Within the playground, you'll find a visual exploration of your API. It reads your `/openapi.json` spec (generated by `hono-openapi` if enabled) to display your API endpoints, such as `/api/tasks` or `/api/tasks/{id}`. You can select an endpoint and fill in parameters, path variables, and request bodies directly in the UI.

This is useful for testing and debugging during development without switching to tools like Postman or cURL.
...
1.  **Set `DATABASE_URL` secret in Cloudflare:**
    ...
    > Steps may vary based on your Cloudflare account and login status. Log in if prompted.
...
You've adapted the `create-honc-app` boilerplate to build a serverless Task API using the HONC stack. You've defined a schema with Drizzle, created Hono endpoints with Zod validation, tested locally with cURL and the Fiberplane API Playground, and deployed to Cloudflare Workers.
