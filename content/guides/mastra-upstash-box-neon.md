...
updatedOn: '2026-09-21T10:11:50.241Z'
---
...
const main = async () => {
    ...
    await box.exec.command("sudo mkdir -p /work/scripts /work/out && sudo chown -R $(whoami) /work");
...
Add `UPSTASH_BOX_API_KEY` to `.env.local` with your [Upstash Box API key](https://upstash.com/docs/box/overall/quickstart#1-get-your-api-key). The script creates a new box, creates the `/work` directories the tools rely on, installs the Python packages, takes a snapshot, deletes the box, and prints the snapshot ID.
...
```
@Data analyst Which region had the lowest and highest revenue last month, and how did it trend week over week?
```

![Mastra data analyst responding in Slack](/docs/guides/mastra-data-analyst-slack-response-2.png)
