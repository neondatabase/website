...
subtitle: Build a React portfolio website with a chatbot powered by pgvector, Lakebase Postgres, FastAPI, and OpenAI.
author: sam-harri
...
In this guide, you will build a full-stack portfolio website using `React` for the frontend and `FastAPI` for the backend, with a Retrieval-Augmented Generation (RAG) chatbot that uses `pgvector` in a Postgres database on Neon to store and retrieve embeddings created with an OpenAI embedding model.

The result is a portfolio that shows front-end and back-end work and uses AI to answer questions about your experience.

## Prerequisites

Before you start, make sure you have the following tools and services ready:
...
- `pip`: This is required for installing and managing Python packages, including [uv](https://docs.astral.sh/uv/) for creating virtual environments. You can check if `pip` is installed by running the following command:
  ...
  pip --version
  ```
- Neon account: You need a Neon account for your Postgres database. If you don't have an account yet, [sign up here](https://console.neon.tech/signup).
...
4.  Install dependencies.
    ...
    - `pgvector` : A Python client for working with pgvector in Postgres
...
In this section, you will enable the `pgvector` extension from the Neon Console's SQL Editor, add the database schema, and create the database connection pool and lifecycle management logic in FastAPI.
...
With your schema in place, you're now ready to connect to your database in the FastAPI application. To do this you must create a `.env` file in the root of the project to hold environment-specific variables, such as your Neon connection string and API keys.
...
Make sure to replace the placeholders (user, password, your-neon-hostname, etc.) with your actual Neon database credentials, which you can copy by clicking **Connect** in the Neon Console, and to fetch the OpenAI key from the OpenAI console.

In your project, the `src/database/postgres.py` file manages the connection to Postgres using `asyncpg` and its connection pool, which reuses database connections instead of opening a new one per request. With this, you can use asynchronous queries, allowing the application to handle multiple requests concurrently.
...
`init_postgres` opens the connection pool to the Postgres database, and `close_postgres` is responsible for gracefully closing all connections in the pool when the `FastAPI` app shuts down to properly manage the lifecycle of the database.
...
A typical RAG chatbot workflow involves first creating an embedding for the user query, then finding the most similar embeddings in the database, and finally using the context of these embeddings to generate a response. Here, embeddings are generated using OpenAI's text-embedding-3-small model, and the similarity between embeddings is calculated using the pgvector `<=>` operator, which returns the cosine distance between two vectors (lower means more similar, which is why the query orders ascending). The chatbot response is then generated using OpenAI's GPT-4o-mini model.
...
3. Update the frontend content.
   ...
   Now, you will update the content of your portfolio, such as your bio, projects, and skills, and experience to match your personal details. Each section in the portfolio is a separate component that you can modify to include your own information.
   These include:
...
Now, with the React app running, take a look at how the website appears. Check that the design, content, and overall presentation are what you expect. You can interact with the chatbot (which will not yet be functional until the backend is also running) to check the layout and form submission.
...
## Optional: Running the full-stack project with Docker and Docker Compose

In this optional section, you will containerize both the frontend and backend of your full-stack portfolio website using Docker. Docker keeps dependencies and setup the same across environments, and Docker Compose runs both services (frontend and backend) with a single command.
...
3. Set up Docker Compose.
   ...
   Docker Compose runs multiple containers together. You can define both the frontend and backend in a single configuration and run them together with a single command.
...
4. Run the application with Docker Compose.
   ...
   - Start the backend container before the frontend container, based on `depends_on`

   You can now run the frontend and backend in containers with a single command. This setup also helps if you plan to deploy to a cloud environment or want others to run the project without manual installation steps.

## Conclusion

You built a full-stack portfolio website with React, FastAPI, pgvector, Lakebase Postgres, and OpenAI, including a RAG chatbot that answers questions about your projects, skills, and experience.
