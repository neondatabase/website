...
subtitle: Automate workflows by connecting Lakebase Postgres to thousands of apps with Zapier, triggering actions from database events or pushing data into Neon from other services.
...
This guide walks you through two common automation scenarios:
...
You can adapt the same patterns to other use cases.

## Prerequisites

Before you begin, make sure you have the following:

- **Zapier account:** A Zapier account is required to create and manage Zaps. The PostgreSQL integration is a Pro feature on Zapier and requires a [paid plan](https://zapier.com/pricing).
...
- **Database tables (for examples):** The examples in this guide use the following tables. Create these tables in your Neon database if you intend to follow along:
...
Before creating Zaps, you need to connect your Neon database to Zapier. Zapier uses a generic "PostgreSQL" app integration. Due to how [Neon uses Server Name Indication (SNI) for routing connections](/docs/connect/connection-errors#the-endpoint-id-is-not-specified) and how some clients handle SNI, the password field in Zapier needs a specific format.
...
4.  A pop-up window will appear asking for connection details. You can find most of these in your Neon Console by clicking **Connect** in the nav for your database. Fill in the following fields:
    ...
    <Admonition type="important" title="Password format for Neon in Zapier">
    ...
    **Example:** If your endpoint ID is `ep-tight-boat-a6aplura` and your password is `MySecurePassword`, you would enter:
    ...
    This format lets Zapier connect to your Neon database without SNI.
    </Admonition>

    <Admonition type="important" title="Security best practice">
    Create a dedicated database role in Neon for Zapier. Grant this role only the minimum permissions it needs on the tables Zapier works with.
    </Admonition>
...
Let's create a Zap that sends a Slack message whenever a new user is added to the `users` table in Neon.

### Step 1: Set up the trigger (new row in Neon)
...
2. **Trigger setup:**
   ...
   - For "Account", select the PostgreSQL connection to Neon you configured earlier. Click "Continue".
   ...
   - **Order By:** Select the column Zapier should use to find new rows. An auto-incrementing primary key like `id` or a timestamp like `signed_up_at` works well. This example uses the `id` column.
3. Click "**Continue**".
...
### Step 2: Set up the action (send a Slack message)
...
1.  **Action setup:**
    ...
    - You can use the "/" button to insert fields from the trigger data. For example, you might write:
      ...
      This inserts the user's name and email from the Neon trigger data. Refer to the image below for an example.
...
Now, whenever a new row is added to your `users` table in Lakebase Postgres, Zapier posts a message to your Slack channel.
...
### Step 1: Set up the trigger (new Google Form response)
...
6.  Click "**Continue**".
7.  Fill out and submit your Google Form once so Zapier has test data to work with.
...
### Step 2: Set up the action (create a row in Neon)
...
Now, every time someone submits your Google Form, Zapier adds the response to your database on Neon.

## Expanding to other use cases

The two examples above show the basic patterns for integrating Neon with other services via Zapier:
...
Here are some other use cases for these patterns:
...
- **Send custom emails from data in Neon:**
  ...
  - Action: Create a personalized email using OpenAI and send it via Mailchimp/Resend etc.

The process for building these Zaps is similar:
...
- **Specific errors:**
  ...
  - **Recommended action:** If you encounter this specific error, and you've confirmed your connection details (including the password format) are correct, **contact Zapier Support directly** as described in the above community post. You can contact them through the [Zapier Support page](https://zapier.com/app/get-help).

## Conclusion

You've connected Zapier to a database on Neon, sent Slack messages when new rows arrive, and logged Google Form responses into a table. Browse the [Zapier templates for PostgreSQL](https://zapier.com/apps/postgresql/integrations#zap-template-list) for more trigger and action combinations.
