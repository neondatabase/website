...
subtitle: How to use row-level security for multi-tenant apps, where it helps and where it hurts, and how branching and authentication on Neon let you safely iterate on RLS policies.
...
If you are building a multi-tenant app on Postgres, [row level security (RLS)](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) sounds like a clean promise: define access rules once in the database, and every query becomes safe by default. The idea is appealing when you have many tables, many endpoints, and many ways to reach the same data. But RLS is not "turn it on and forget it". It changes how your application authenticates, how your queries are shaped, how you debug production issues, and how confident you can be that a new feature did not pass through your data boundary.

So the question to ask is whether RLS is the right tool for your system, given your team’s workflow, the kinds of queries you run, and how you ship iterations of your application. Neon does not change what RLS is, but it makes it easier to test policy changes against real application behavior before they reach production.
...
To enable RLS, you define a policy at the database level to enforce tenant isolation, for example:
...
With RLS, the application no longer needs to manually filter by `account_id` in each query. Instead, it can write:
...
RLS is also a strong fit when you have multiple query surfaces that touch the same tables. If you have a REST API, server actions, background jobs, and internal tooling all using the same database, enforcing row access in one place becomes more valuable. The app layer isn't untrustworthy, but the number of places where access logic can spread is large. The more ways your app accesses data, the more useful it is to have a single place to enforce access rules.
...
These policies are hard to read and debug, and they need maintenance every time the requirements or the rules that touch data outside Postgres change.
...
Now, every read on `project_members` runs a subquery and a function call per row. On large tables, evaluating the subquery for each row can be much slower than a simple indexed lookup.
...
Policy changes are hard to test safely in production, so you need production-like staging environments to catch insecure or misconfigured RLS setups before they ship.
...
Make sure every connection, including jobs and tasks, sets the needed context or uses the correct restricted role. Otherwise, RLS can't protect your data.
...
## How Neon changes the trade-offs

Neon cannot write correct policies for you. What it can do is give you a **production-like loop**: branch your database in seconds, clone both app data and auth state, point a preview app at that branch, then run real logins and queries under the same roles and session shape you use in production.

[Branching](/docs/introduction/branching) creates an isolated copy of your database, including all schema, data, and system tables, without affecting production. Each [branch](/docs/manage/branches) has its own compute and connection string, so you can test schema changes, migrations, or RLS policy updates against production-like data.
...
From the [Neon Console](https://console.neon.tech), open the new branch and copy its **connection string** for the role your app uses in production (for example a restricted `app_user`, not the owner role used only for migrations).

### Step 2: Point your app preview environment at the branch

Each Neon branch has its own compute endpoint. If you use Managed Better Auth, each branch also gets [its own Auth Base URL](/docs/auth/branching-authentication). Preview and CI environments must use that branch’s URL (not the production branch’s URL), because sessions and tokens are scoped to the [Auth endpoint](/docs/auth/guides/manage-auth-api) for that branch.
...
Now, sign in on the preview hostname or localhost configuration that targets this branch. Cookies issued there belong only to the new branch's Auth endpoint, which mirrors how you isolate real previews from production.
...
In your application code, **the important part is unchanged**: resolve the user with Managed Better Auth, set tenant or user context for RLS (for example via `set_config` or your pool’s `SET` hook), and run the queries your routes and jobs actually use. Because the branch contains cloned users and membership rows, joins inside policies and "wrong role" mistakes **show up here before production**.

### Step 4: Run end-to-end tests and delete the branch when done

RLS changes can break things you didn’t expect. A dedicated branch lets you run full end-to-end tests (like with [Playwright](https://playwright.dev/)) or do manual QA on real user scenarios (logins, API calls, background jobs) without putting production data at risk. Point your preview environment at the new branch with the right environment variables. When you’re done, [delete the branch](/docs/manage/branches#delete-a-branch) so it stops using storage and compute. You can do this either in the Neon Console or right from your terminal using the Neon CLI:
...
Neon makes RLS safer to adopt by letting you test policy changes on isolated branches with realistic data, users, and workflows before they reach production. You still need to write correct policies, but you find out whether they work before your users do.
