...
Neon removes that trade‑off with [branching](/docs/introduction/branching). In seconds, you can create an isolated copy of your production database identical in data volume and distribution, but backed by its own compute resources. This makes it possible to run high‑concurrency load tests against a Neon branch without any risk to production.
...
Instead of pointing your test traffic at the `production` branch, you will create an isolated branch. That way, the requests generated by k6 don't compete with production traffic for compute or skew production metrics.

In the Neon Console:

1. Open your project and select **Branches** under **Project**.
2. Click **New branch**.
...
6. Leave **Automatically delete branch after** checked (1 day by default) or pick a duration that covers your testing.
7. Click **Create**.

After the branch is created, you're shown the connection string for `load-test-branch`. Update your `.env` file to use this new connection string:

```env
DATABASE_URL="postgresql://alex:AbC123dEf@ep-cool-darkness-123456.us-east-2.aws.neon.tech/postgres?sslmode=require&channel_binding=require"
```

Start your API server. It's now connected to the isolated branch.
...
The test failed. With a `p(95)` of `193.74ms`, the response times are almost four times the `50ms` threshold you set. The database contains 500,000 rows and there is no index on the `category` column, so Postgres performs a sequential scan, reading every row for each request from the 50 virtual users. Under load, those scans saturate CPU and slow every request.
...
The ideal index for this query is on `(category, created_at DESC)`. It lets Postgres filter by category and retrieve the most recent products without scanning the entire table.
...
This time, the `p(95)` latency should drop well below the threshold, and the test should pass.
...
4. Delete `load-test-branch` to clean up the temporary resources, or let it expire if you set an auto-delete duration.
5. In the Neon Console, select **Branches** under **Project**, select `load-test-branch`, then click the **More** drop-down menu on the branch overview page and select **Delete**.
...
You branched a database with 500,000 rows, load tested it with k6 on the branch's own compute, and cut `p(95)` latency from about 194ms to 6ms with a composite index, without touching production.

## Next steps

To maximize the value of this workflow, automate it. Provision and tear down Neon branches in CI, run your load tests, and automatically fail builds on latency or error-rate regressions. Running it as a routine check, especially before major schema changes, query rewrites, or ORM upgrades, catches performance regressions before they reach production.
