...
Some work shouldn't or can't happen inside a single HTTP request-response cycle: multi-step AI enrichment pipelines, customer onboarding sequences, background uploads, and third-party webhooks. Production backends need a way to run it in the background.

In traditional architectures, background work forces a tradeoff:

- **Fragile serverless execution**: Running multi-step tasks inside standard serverless functions is brittle. Short execution limits cut off long runs mid-stream. If step 3 of a 5-step pipeline fails due to a network hiccup or rate limit, the entire function crashes. Re-running the function duplicates earlier database writes and burns through expensive AI API tokens, while building your own state tracking in the database is hard to maintain.
...
The root cause is that serverless compute has historically lacked **step-level durability**. If a single step in a multi-step workflow fails, the entire workflow fails, and you have to manually recover state and re-run the workflow.

This guide addresses that by combining [Inngest](https://www.inngest.com) with [Neon Functions](/docs/compute/functions/overview). Inngest provides an event-driven durable execution engine that turns code into checkpointed steps orchestrated over standard HTTP. Neon Functions provides long-running Node.js compute in the same region as your [Lakebase Postgres](/docs/postgres/overview) database, with [Neon AI Gateway](/docs/ai-gateway/overview) credentials injected automatically.
...
In this tutorial, you will build an automated lead enrichment pipeline that receives a customer signup event, writes initial state to Lakebase Postgres, researches the company using a web search tool to generate an executive summary, executes a durable delay, and updates the database record upon completion. You can reuse the same pattern for multi-step AI agents, human-in-the-loop approval flows, scheduled reporting jobs, or any workflow that needs durable retries.
...
3. **In-process persistence and web search**: The handler executes queries against **Lakebase Postgres** using `pg` and uses a mock web search tool to simulate company research and generate an executive summary.
...
2. **Neon account**: Sign up at [console.neon.tech](https://console.neon.tech/signup). This guide calls a model through [AI Gateway](/docs/ai-gateway/overview), which is available on paid plans (Launch and Scale) with prepaid credits.
...
4. **Inngest account**: Sign up for a free account at [inngest.com](https://www.inngest.com).
   ...
   You can also self-host Inngest using the [Inngest self-hosting guide](https://www.inngest.com/docs/self-hosting). Use Lakebase Postgres as the backing database for Inngest's durable state storage. The workflow code in this guide works the same with Inngest Cloud or a self-hosted Inngest instance.
   </Admonition>
...
- `pg`: Node.js Postgres client for communicating with Lakebase Postgres.
...
Select **AWS US East (Ohio)** (`aws-us-east-2`), **AWS US East (N. Virginia)** (`aws-us-east-1`), **AWS Europe (Frankfurt)** (`aws-eu-central-1`), or **AWS Asia Pacific (Singapore)** (`aws-ap-southeast-1`) when creating your Neon project; this guide uses US East (Ohio). Neon Functions are currently available in these regions. Support is expanding toward [all regions](/docs/introduction/regions). Select **Yes** when prompted to manage your setup as code (`neon.ts`).
</Admonition>

`neon link` creates a `.env.local` file in your project root with your Neon project details, including `DATABASE_URL` and other Neon-specific variables.
...
Neon Functions are long-running, so you can keep a connection pool across invocations instead of opening a new database connection on every request.
...
The `id` field identifies your application within Inngest. When you register this function with Inngest Cloud, the ID ties your deployed function to your Inngest account so events are routed to it.
...
export const processLeadWorkflow = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        // Step 1: Initialize record in Lakebase Postgres
...
- **Step 3 (`wait-for-processing-window`)**: Sleeps durably for 5 seconds. Inngest holds the timer, so your function isn't running or billed during the wait. This simulates a processing window, for example waiting on external API results or a human review cycle.
...
- **`serve()` from `inngest/hono`**: This is Inngest's adapter for Hono. It creates a set of route handlers that implement the [Inngest serve protocol](https://www.inngest.com/docs/learn/serving-inngest-functions#framework-hono). Inngest communicates with your function over HTTP using three methods: `GET` for configuration discovery, `PUT` for registering the function, and `POST` for executing steps. The `serve` function handles all of this.
...
- **`env`**: Passes the Inngest keys from your `.env.local` file to the function at runtime. Neon injects other Neon-specific environment variables, like `DATABASE_URL`, automatically.
...
4. Your `process-lead-workflow` function appears in the Inngest dashboard, along with its trigger event (`app/lead.created`).
...
The `status` column shows `completed` and the `summary` column contains the AI-generated executive summary. All four steps ran: the initial insert, the web search and AI synthesis, the durable sleep, and the final update.

If a step had failed (for example, due to a transient API outage), Inngest would have retried that specific step up to 3 times without re-executing the earlier database insert. After fixing any issues, you can replay failed runs from the [Inngest Cloud dashboard](https://app.inngest.com).
...
This lead enrichment pipeline is a starting point. To build on it, swap in a real search provider (see above), add more steps, or trigger the workflow from your app's signup handler. The [Inngest docs](https://www.inngest.com/docs) cover patterns like fan-out, cron triggers, and waiting for events.
