...
You need a project in AWS US East (Ohio) (`aws-us-east-2`) or AWS Europe (Frankfurt) (`aws-eu-central-1`). Support is expanding toward all regions. Using the AI Gateway requires a paid Neon plan with prepaid credits, which gives you the generally available models. To request access to the full foundation model catalog, see [Model access](/docs/ai-gateway/overview#model-access).
