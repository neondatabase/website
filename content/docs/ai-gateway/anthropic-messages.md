...
Sending a non-Anthropic model ID returns `400 model "gpt-5" is not available on the anthropic_messages endpoint`. Use the [chat completions endpoint](/docs/ai-gateway/chat-completions) if you need to call multiple providers from the same code.
...
| `400 Bad Request` | `model "gpt-5" is not available on the anthropic_messages endpoint` | Non-Anthropic model sent to this endpoint |
