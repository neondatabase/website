...
updatedOn: '2026-06-10T08:45:19.666Z'
---
...
`neon psql` uses the native `psql` binary from your `$PATH` if one is available. If not, it falls back to a built-in TypeScript implementation automatically — no separate PostgreSQL client tools installation required. The built-in implementation supports SCRAM-SHA-256 authentication and standard interactive and non-interactive usage.
