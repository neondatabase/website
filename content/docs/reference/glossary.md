...
A periodic load generated by the Control Plane to determine if a compute can start and read and write data. The availability checker queries a system database without accessing user data. Control Plane logic controls how often the availability checker runs to avoid too many concurrent availability checks when computes are started simultaneously.
