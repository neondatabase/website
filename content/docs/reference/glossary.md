...
A named collection of database objects. A Neon project has a ready-to-use database, named `neondb` by default, that resides in the default `public` schema. A Neon project can contain multiple databases. Users cannot manipulate system databases, such as the `postgres`, `template0`, or `template1` databases.
