...
A [copy-on-write](#copy-on-write) clone of your data created from a current or past state. A branch can be created from the root branch of your Neon project or from a previously created branch. The data in a branch can be modified independently from its originating data. See [Branching](/docs/conceptual-guides/branching/). Each branch has a dedicated [endpoint](#endpoint), which is the compute instance associated with the branch.
...
The default data branch created with each Neon project. The root branch for a Neon project is called `main`, not to be confused with the default database created with each Neon project, which is also called `main`. A root branch cannot be deleted from a project.
