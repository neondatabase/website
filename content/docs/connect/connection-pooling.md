...
Connection pooling supports up to 10,000 concurrent connections. However, these connections are not a magic bullet: These connections are pooled and take shared ownership of a pool of connections to PostgreSQL. This pool is shared between all connections that connect with the same user credentials to the same database, and share a total of up to [64 connections](#neon-pgbouncer-configuration-settings). This means any user can only have 64 concurrently active transactions in a single database when connected through the pooler endpoint.  
Additionally, because PgBouncer still needs to connect to PostgreSQL, the number of active concurrent connections is further limited by `max_connections`: if the `max_connections` parameter of Postgres is 112 (i.e. the endpoint was started with 0.25 CU), you can still hit PostgreSQL's own connection limit with only 2 roles.  
If your application connects to your database using a single Postgres role, expect to encounter these limits when you start exceeding 64 concurrent active connections. For information about creating additional Postgres roles, see [Manage roles](/docs/manage/roles#manage-roles-with-sql). Roles can created via the Neon Console, CLI, API, and SQL.
</Admonition>
