...
Connection pooling supports up to 10,000 concurrent connections. However, this does not increase the number of active transactions: For a single user connecting to a single database, the maximum number of concurrent active connections is capped to 64 (see below) to keep the database available for administrative direct access by the user. Additionally, as PgBouncer still needs to connect to PostgreSQL, the number of active concurrent connections is further limited by `max_connections`: if the `max_connections` parameter of Postgres is 112 (i.e. the endpoint was started with 0.25 CU), you can saturate PostgreSQL's connections by connecting with only 2 roles.  
If your application connects to your database using a single Postgres role, expect to encounter this limit when you start exceeding 64 concurrent active connections. For information about creating additional Postgres roles, see [Manage roles](/docs/manage/roles#manage-roles-with-sql). Roles can created via the Neon Console, CLI, API, and SQL.
</Admonition>
