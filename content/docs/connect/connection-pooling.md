...
Connection pooling supports up to 10,000 concurrent connections. However, these connections are not a magic bullet: These connections are pooled and take shared ownership of a pool of connections to PostgreSQL. This pool is shared between all connections that connect with the same user credentials to the same database, and share a total of up to [64 connections](#neon-pgbouncer-configuration-settings). This means any user can only have 64 concurrently active transactions when connected through the pooler endpoint.  
