...
This error may also occur when a connection is established less than 500ms before a compute endpoint suspends due to inactivity. Neon monitors for activity every 500ms. In this case, Neon is not aware of the new activity and proceeds with the compute suspension operation, resulting in the connection being terminated.
