...
Upsert is a combination of [update](/docs/postgresql/postgresql-update) and [insert](/docs/postgresql/postgresql-tutorial/postgresql-insert). The upsert allows you to update an existing row or insert a new one if it doesn't exist.
...
- `ON CONFLICT (conflict_column):` This clause specifies the conflict target, which is the [unique constraint](/docs/postgresql/postgresql-unique-constraint) or [unique index](/docs/postgresql/postgresql-indexes/postgresql-unique-index) that may cause a conflict.
