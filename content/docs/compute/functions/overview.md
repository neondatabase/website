...
> Functions are in beta and available only in **AWS US East (Ohio) (`aws-us-east-2`)**, so create your project there to use them. Functions are free to use during beta, subject to usage limits, on any plan. See [plans and pricing](/docs/introduction/plans#functions) for the rates that apply when billing begins.
