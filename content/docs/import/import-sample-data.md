...
The sample data sets are listed in order of smallest to largest installed size. Please be aware that the Neon Free Tier has a storage limit of 3 GBs per branch. Free Tier users will not be able to install datasets larger than 3 GBs.
...
| [Wikipedia vector embeddings](#wikipedia-vector-embeddings) | 1  | 25000 | zip: 667 MB, csv: 1.7 GB | 850 MB         |
| [Postgres air](#postgres-air-database)              | 10     | 67228600     | 1.2 GB                   | 6.7 GB         |

<Admonition type="note">
Installed size is measured using the query: `SELECT pg_size_pretty(pg_database_size('your_database_name'))``. The reported size for small datasets might appear larger than expected due to inherent PostgreSQL storage overheads, such as system metadata, MVCC bloat, and reserved space allocations.
</Admonition>
...
| Tables | Records | Source file size         | Installed size  |
|--------|---------|--------------------------|-----------------|
| 1      | 118     | 17 KB                    | 7176 kB         |
...
The Chinook database is a sample database for a digital media store, including tables for artists, albums, media tracks, invoices, customers, and more.
