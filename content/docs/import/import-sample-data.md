...
- A Neon [Pro](/docs/introduction/pro-plan) account if you intend to install a dataset larger than 3 GiBs.
...
Sample datasets are listed in order of the smallest to largest installed size. Please be aware that the Neon Free Tier has a storage limit of 3 GiBs per branch. Datasets larger than 3 GiBs cannot be loaded on the Free Tier.
