...
- A **connection string** to your current database in this format:
  ...
  If you are migrating a database from one Neon project to another, you need the connection string for the source database, which you can access from the **Connect** modal (**Connect** in the Console nav). When the source is a Neon project, use a direct (unpooled) connection string, not a pooled one. Make sure the hostname does not include the `-pooler` suffix. See [Connection pooling](/docs/connect/connection-pooling).
