...
3. Load the database dump into Neon using `pg_restore`. For example:
    ...
    - `--no-owner` and `--no-acl`: These options prevent restoration of ownership and access privileges statements for database objects, which will cause non-fatal errors during the restore operation. For more information, see [Ownership and privilege considerations](#ownership-and-privilege-considerations).
...
- _Option 1_: Exclude database object ownership and privilege assignment statements from being executed when you restore data by specifying the `--no-owner` and `--no-acl` options with `pg_restore`. The [pg_dump with pg_restore](#pg_dump-with-pg_restore) example above uses these options with `pg_restore`.
- _Option 2_: Ensure that the Neon role running the restore operation owns all database objects, and that any role that may be referenced by `GRANT/REVOKE` statements in your dump archive are created in Neon prior to restoring data. This method preserves privileges on database objects but not ownership. To use this method:
