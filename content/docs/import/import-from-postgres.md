...
- If your database is small (< 1 GiB), you can pipe `pg_dump` output directly to `pg_restore` to save time. See [Pipe pg_dump to pg_restore](#pipe-pg_dump-to-pg_restore).
...
For small databases (< 1 GiB), the standard output of `pg_dump` can be piped directly into a `pg_restore` command to minimize migration downtime:
...
Piping is not recommended for medium (> 1 GB) and large databases (> 5 GiB), as it is susceptible to failures during lengthy migration operations.
...
- `-j <njobs>`: Consider this option for medium (>1 GiB) and large databases (>5) to dump tables in parallel. Set `<njobs>` to the number of available CPUs. Refer to the [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html) documentation for more information. In Neon, this option only make sense for Pro plan users who can configure computes with >1 vCPU.
...
- `-j <njobs>`: Consider this option for medium (>1 GiB) and large databases (>5) to run the restore process in parallel. Set `<njobs>` to the number of available vCPUs. Refer to the [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html) documentation for more information. In Neon, this option only makes sense for Pro plan users who can configure computes with >1 vCPU. It cannot be used together with `--single-transaction`.
