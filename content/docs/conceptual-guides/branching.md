...
Any branch action that you can perform in the Neon Console can also be performed using the Neon API. The following examples demonstrate how to create, view, and delete branches using the Neon API. For a complete list of branch-related API methods, refer to the [Neon API reference](https://neon.tech/api-reference/).

A Neon API request requires an API key. For information about obtaining an API key, see [Using API Keys](/docs/get-started-with-neon/using-api-keys).

`<project_id>` and `<branch_id>` values can be found in the Neon Console on the **Settings** tab, under **General Settings**. Project and branch IDs generated by Neon will appear similar to `restless-sea-792742` and `branch-broken-paper-301953`, respectively.
...
- **Listing branches**
    ...
    A cURL command with the `<project_id>` values specified appears similar to the following:
...
- **Deleting a branch**
    ...
    As a cURL command, the API request appears as follows:
