...
The developer experience across our entire platform is rooted in the lakebase architecture and anchored around four core pillars:
...
Storage in Lakebase Postgres is also [history-preserving](https://neon.com/blog/get-page-at-lsn) by design. Same with the files in [Neon Object Storage](https://neon.com/docs/storage/overview). As data changes over time, past versions of your database state are retained efficiently as part of normal operation, making operations that are painfully slow in traditional backends (like restores) trivial on Neon.
...
You can build your branching workflows using the [CLI](/docs/cli), [API](/docs/reference/api), or [GitHub Actions](/docs/guides/branching-github-actions). You can also keep your development branches up-to-date by resetting your schema and data to the latest from `main` with a simple command.
...
- **Branch per developer**. Each engineer works against their own isolated backend branch environment, avoiding conflicts when making schema or data changes.
...
That depends on two things working together.
...
Every core operation in the Neon platform is available programmatically:
- Our [API](/docs/reference/api) is shaped by real-world agent and platform workloads: create and manage hundreds of thousands of projects, automate lifecycles with minimal human intervention, and enforce usage limits and cost controls (compute uptime, autoscaling bounds, data written, storage per branch, and more)
- Our [CLI](/docs/cli), GitHub Actions, Vercel preview branching, MCP, and Agent Skills all sit on that same surface, so humans and agents share one control plane
...
Backend workflows stop being the exception you have to still manage manually. Developers can delegate them to coding agents, and agent platforms can treat Neon as infrastructure they drive end to end - with the lakebase architecture keeping that automation fast and affordable.

## Composable and open

Neon is a complete set of cloud backend primitives built around Lakebase Postgres: database, auth, object storage, compute functions, data API, and AI gateway. It gives you a full backend for apps and agents, but as open, composable pieces on open standards, not a locked-in all-in-one backend where every layer is proprietary and inseparable.
