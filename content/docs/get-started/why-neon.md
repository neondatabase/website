...
summary: >-
  ...
  add-ons. The Neon platform also comes with Managed Better Auth, Object Storage, Functions, and AI Gateway. 
...
Traditional Postgres providers scale by moving VMs up and down, placing instances behind proxies, or by manual tuning. Lakebase Postgres does none of that. Instead, the lakebase architecture is serverless, which to us means:
...
The lakebase architecture lets Lakebase Postgres behave the way developers expect modern infra to behave: instant, intuitive, cost-efficient, and safe to experiment with. This modernizes workflows that, in other managed Postgres services, still feel decades old.
...
- **A Free Plan developers can actually use.** The lakebase architecture makes it incredibly efficient to run a large Free Plan with many projects per account and enough resources to build real apps.

## Who uses Neon and why

### Developers via agents: From side projects to live apps

Independent developers want to build without friction. They don’t want to create accounts, configure VMs, or invest large sums just to test an idea - they want their agent to directly handle their infra. 

**Why they build on Neon**

- Our platform is agent-first: agents are a first-class interface
- The Free Plan is generous enough to build real apps, not just to test things out
- A Postgres connection string is available immediately, no provisioning waiting times
...
Startups want to ship product fast and avoid cloud infrastructure complexity. They need their Postgres to be reliable, scalable, and invisible, something they never have to think about unless something goes wrong, and they need their coding agent to handle everything reliably. 

**Why they build on Neon**

- The lakebase architecture removes cumbersome DBA tasks
- Those tasks that still exists can be handled by agents
...
This is a specialty use case for Neon: full-stack codegen platforms that need to spin up thousands of independent applications instantly, each with its own backend. They need a backend platform that can support a fleet of thousands of mostly inactive apps every day without breaking performance or blowing up costs.

**Why they build on Neon**

- Neon is already tested at scale, powering platforms like Replit
- They can deploy a Postgres backend instantly and transparently on behalf of the end-user, including database, auth, AI models, functions, and blob storage
- Thousands of short-lived, low-usage databases are deployed / deleted by APIs / agents  
- Scale-to-zero makes this setup economically viable at scale
- Branching makes it simple to build versioning, rollbacks, checkpoints

<Admonition type="tip" title="Neon Agent Plan">
  Building a full-stack agent that deploys backends? Apply to our [Agent Plan](https://neon.com/programs/agents#agent-plan-pricing) and get access to special pricing, resource limits, features, and support from the team.
</Admonition>

## The architecture that makes it possible: lakebase architecture

The benefits developers experience with the Neon platform (instant branching, autoscaling, scale-to-zero, and fast recovery) are not product features layered on top of Postgres. They fall out naturally from the lakebase architecture.
...
Separation alone is not enough. Lakebase Postgres’s most distinctive capabilities come from its versioned storage engine, which preserves the full history of the database. Every WAL record and every page version is retained in a single, unified system. As a result:
