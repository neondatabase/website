...
updatedOn: '2026-06-15T09:54:16.531Z'
---
...
| [pg_ivm](https://github.com/sraoss/pg_ivm)                                                       |     1.9 |     1.9 |       1.9 |       1.9 |      1.12 | **Not available for new installs.** Existing databases that already installed `pg_ivm` are unaffected. On PG14–17, use `SELECT create_immv(...)` (function in `pg_catalog`). On PG18, use `SELECT pgivm.create_immv(...)` or add `pgivm` to your `search_path`. For new projects, use standard [materialized views](/docs/postgres/rules-materializedviews).                                                                                                  |
