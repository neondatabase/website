...
subtitle: Convert PostgreSQL Write-Ahead Log (WAL) to JSON format
...
<Admonition type="note">
    The `wal2json` plugin is included in your Neon project and doesn't require a separate installation. Detailed configuration information can be found in the [Neon Logical Replication Guide](https://neon.tech/docs/guides/logical-replication-guide).
</Admonition>

**Version availability:**

The `wal2json` plugin is available in all PostgreSQL versions supported by Neon. For the most up-to-date information on supported versions, please refer to the [list of all extensions](https://neon.tech/docs/extensions/pg-extensions) available in Neon.
...
To start using `wal2json`, you first need to create a replication slot. You can do this by running the following query:
...
Run the following query to create the `inventory` table, and insert some sample data to it:
