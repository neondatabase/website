...
| [Snapshots](#snapshots)                               | 1 manual snapshot limit (free)             | 10 manual snapshot limit; $0.09/GB-month for manual and scheduled snapshot storage starting May 1, 2026 | 10 manual snapshots limit; $0.09/GB-month for manual and scheduled snapshot storage starting May 1, 2026 |
...
Snapshots capture the state of your branch at a point in time. You can create snapshots manually or schedule automated snapshots.

The per-plan snapshot limit applies to **manual snapshots only**. On paid plans, snapshots created by backup schedules do not count toward this limit (backup schedules are not available on the Free plan).
