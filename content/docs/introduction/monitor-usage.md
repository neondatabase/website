...
- **Storage** includes your data size and history. Neon maintains a history of changes to support branching-related features such as [instant restore](/docs/reference/glossary#point-in-time-restore). The Launch plan supports up to 7 days of restore window, the Scale plan allows up to 14 days, and the Business plan offers up to 30 days. The default is 1 day on all plans. Keep in mind that restore window increases storage. More history requires more storage. To manage the amount of history you retain, you can configure the restore window setting for your project. See [Configure restore window](/docs/manage/projects#configure-restore-window).
