...
- **Compute**: The total number of [compute hours](/docs/reference/glossary#compute-hours) used during the current billing period. Compute usage is reset to zero at the beginning of each month. For example, on the Launch plan, compute usage will be set back to **0/300h** at the beginning of each month. On the Free Tier, you have a 5h compute allowance, but this only applies to child branch computes. On the Free Tier, your [primary branch](/docs/reference/glossary#primary-branch) compute is always available and is therefore not tracked by this metric.  
