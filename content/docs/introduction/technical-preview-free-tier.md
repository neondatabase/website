...
- 100 hours of Free Tier compute per month (total). Neon scales Free Tier compute to zero after 5 minutes of inactivity. Only active time of Free Tier compute is counted toward the limit. When you reach the limit, Free Tier compute endpoints assigned to [non-primary branches](/docs/reference/glossary#non-primary-branch) are unavailable until the limit is reset at the beginning of the next month. You are always able to connect to the compute endpoint assigned to the primary branch, regardless of the limit. This ensures that access to data on the primary branch is never interrupted.
