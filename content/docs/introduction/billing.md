...
- **History**
  ...
  - _Point-in-time restore_ is the ability to restore data to an earlier point in time. Neon retains a 7-day history in the form of WAL records for a Neon project, by default. [Neon Pro plan](/docs/introduction/pro-plan) users can configure the retention period. WAL records that age out of the retention period are evicted from storage and no longer count toward project storage.
