...
summary: >-
  Postgres character types `CHAR(n)`, `VARCHAR(n)`, and `TEXT` store
  fixed-length, length-limited, and unlimited strings respectively. `CHAR(n)`
  space-pads shorter values; `TEXT` has no length restriction. Use `VARCHAR(n)`
  or `CHAR(n)` when input validation against a maximum length matters, and
  `TEXT` for large variable-length content such as blog posts or product
  descriptions.
