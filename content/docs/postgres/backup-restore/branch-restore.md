...
Object Storage is not part of the database timeline, so a database restore does not revert its contents. The same is true of Functions. This differs from [Reset from parent](/docs/guides/reset-from-parent), which does reset a branch's Object Storage to the parent's current state. Object Storage reset is currently available in two of the four backend regions, with support for the other two coming soon.
