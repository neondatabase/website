...
isDraft: false
redirectFrom: ['/faqs/rotate-database-connection-string-security']
---
...
If you can't change every consumer at once, create a second Postgres role with its own password, point new consumers at it, and drop the old role only after you've confirmed nothing still uses it. See the alternative approach in [How do I rotate my database URL or connection string?](/docs/faqs/rotate-database-url-connection-string).
