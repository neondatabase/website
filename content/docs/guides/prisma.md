...
```text
DATABASE_URL=postgres://daniel:<password>@ep-damp-cell-18160816-pooler.us-east-2.aws.neon.tech/neondb
```
...
- In summary, to use a pooled Neon connection with Prisma client, you require both a pooled Neon connection string (one that includes `-pooler` in the connection string) _and_ the `?pgbouncer=true` flag (required by Prisma Client). See the example above.
