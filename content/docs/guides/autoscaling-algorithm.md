...
For best performance, keep your entire dataset (or at least your working set) in memory by setting your minimum compute size accordingly. For example, a 6.4 GB database can comfortably fit within a compute size of 2 vCPU with 8 GB of RAM (where the Local File Cache, or LFC, can use up to 75% of the available RAM).

However, be aware that when a compute scales down, the LFC is cleared. This means previously cached or pre-warmed data will be lost, and the cache will need to be repopulated as queries are run. For advice on minimizing the impact of cache loss, see [How to size your compute](/docs/manage/endpoints#autoscaling-considerations).
</Admonition>
