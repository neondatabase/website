...
subtitle: Store files via Cloudinary and track metadata in Lakebase
summary: >-
  ...
  metadata (public_id, secure_url, resource_type) in a Lakebase Postgres table.
...
This guide demonstrates how to integrate Cloudinary with Neon. You'll learn how to securely upload files directly from the client-side to Cloudinary using signatures generated by your backend, and then store the resulting asset metadata (like the Cloudinary Public ID and secure URL) in your database.
...
We need a table to store metadata about the assets uploaded to Cloudinary.
