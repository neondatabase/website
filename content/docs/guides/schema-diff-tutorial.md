...
1. Connect to the `people` database with psql:
   ...
   ```bash shouldWrap
   psql 'postgresql://neondb_owner:*********@ep-crimson-frost-a5i6p18z.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require'
   ```
...
2. Retrieve your database connection string using [Get connection URI](https://api-docs.neon.tech/reference/getconnectionuri) endpoint, specifying the required `project_id`, `branch_id`, `database_name`, and `role_name` parameters.
   ...
   ```json
   {
     "uri": "postgresql://alex:*********@ep-green-surf-a5yaumj3-pooler.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require"
   }
   ```
...
3. Connect to the `people` database with `psql`:
   ...
   ```bash shouldWrap
   psql 'postgresql://alex:*********@ep-green-surf-a5yaumj3-pooler.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require'
   ```
...
1. Verify the schema
   ...
   1. Get the connection string for the `people` database on branch `feature/address` using the CLI.
      ...
      ```bash
      postgresql://neondb_owner:*********@ep-hidden-rain-a5pe72oi.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require
      ```

   1. Connect to `people` using psql.

      ```bash
      psql 'postgresql://neondb_owner:*********@ep-hidden-rain-a5pe72oi.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require'
      ```
...
1. Retrieve the database connection string for the `feature/address` branch using [Get connection URI](https://api-docs.neon.tech/reference/getconnectionuri) endpoint:
   ...
   ```json
   {
     "uri": "postgresql://alex:*********@ep-hidden-sun-a5de9i5h-pooler.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require"
   }
   ```
...
1. Connect to the `people` database on the `feature/address` branch with `psql`:
   ...
   ```bash shouldWrap
   psql 'postgresql://alex:*********@ep-hidden-sun-a5de9i5h-pooler.c-2.us-east-2.aws.neon.tech/people?sslmode=require&channel_binding=require'
   ```
