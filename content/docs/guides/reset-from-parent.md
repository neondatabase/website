...
<!-- TEMP region note: when Object Storage reset reaches all four backend regions, remove this Admonition and the "currently available in two of the four backend regions" caveats in storage/overview.md, storage/buckets.md, concepts/branch-your-backend.md, and postgres/backup-restore/branch-restore.md -->

<Admonition type="note" title="Object Storage reset is available in some regions">
Object Storage reset is currently available in two of the four backend regions: AWS US East (Ohio) (`aws-us-east-2`) and AWS Europe (Frankfurt) (`aws-eu-central-1`). Support for the other two, AWS US East (N. Virginia) (`aws-us-east-1`) and AWS Asia Pacific (Singapore) (`aws-ap-southeast-1`), is coming soon. In those two regions, a reset still rolls back Postgres, but the branch's buckets and objects are left unchanged. Resetting the database is available in all regions.
</Admonition>
...
<TabItem>
To reset a branch to its parent using the API, use the [Restore branch](/docs/reference/api/branches/restore-project-branch) endpoint, specifying the parent branch ID as the `source_branch_id`:
