...
1. Enter the hostname, port number, and name for your Postgres database. You can obtain these details from your Neon connection string, which you'll find in the **Connection Details** widget on the **Dashboard** of your Neon project. For example, given this connection string:

    ```bash
    postgres://alex:AbC123dEf@ep-cool-darkness-123456.us-east-2.aws.neon.tech/dbname?sslmode=require
    ```

    Enter the following details in the Airbyte **Create a source** dialog:
...
2. Under **Optional fileds**, you can list each of the schemas you want to sync. These are case-sensitive, and multiple schemas may be entered. By default, `public` is the only selected schema.
3. Select an SSL mode. You will most frequently choose `require` or `verify-ca`. Both of these always require encryption. The `verify-ca` mode  requires a certificate. Refer to [Connect securely](/docs/connect/connect-securely) for information about the location of certificate files  you can use with Neon.
4. Select **Logical Replication (CDC)** from available replication methods, and enter the replication slot and publication you just created.
5. In the **Replication Slot** field, enter the name of the replication slot you created previosly (`airbyte_slot`).
6. In the **Publication** field, enter the name of the publication you created previously (`airbyte_publication`).
    ![Airbyte Create a source](/docs/guides/airbyte_cdc_advanced_fields.png)
...
![Airbyte Create a source](/docs/guides/airbyte_destinations.png)