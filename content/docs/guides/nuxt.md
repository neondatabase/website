...
updatedOn: '2026-08-06T12:49:07.523Z'
---
...
If you haven't signed in to the CLI yet, run `neon auth` first. Replace `main` with your default branch name if it differs (Console-created projects use `production`); run `neon branches list` to check. `neon env pull` defaults to `.env.local`, but `nuxt dev` only loads `.env`, so pass `--file .env`.
