...
summary: >-
  Magic Link is a Managed Better Auth plugin that lets users sign in without a
...
Build a custom magic link flow using the [Neon SDK](/docs/reference/javascript-sdk). Call `signIn.magicLink()` with the user's email and a callback URL. Managed Better Auth sends the email and redirects the user to `callbackURL` after they click the link.
