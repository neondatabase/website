...
> _On the Backup & Restore page there are snapshot cards. Each snapshot card has an ellipsis/’more’ icon button. Each of these buttons opens a menu with a single ‘Delete’ action item. If the user selects Delete, their snapshot will be deleted forever._
>
> _<br />Add a modal dialog after the user clicks delete with a confirmation message and two buttons: ‘Cancel’ and ‘Delete’_. _The copy in the modal must be ‘This is a permanent operation. Deleted snapshots cannot be recovered.’ and must be styled as standard body text. Use other modals for reference._
