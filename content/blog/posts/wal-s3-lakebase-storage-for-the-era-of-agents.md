...
Once the durable record of a transactional database lives in object storage, it stops being locked inside one engine's private format on one engine's disks. Other engines can read it. That is the basis for what we call [LTAP, for Lake Transactional/Analytical Processing](https://www.databricks.com/blog/lakebase-ltap-rethinking-database-storage): instead of two copies of the data in two formats kept in sync by a pipeline, there is one durable copy in open columnar formats that both the transactional and analytical sides read.
