...
- When RLS is applied to functions or views, you have to think about whether to define them with security_invoker as true or false. This will change whether RLS is checked for the role that _defined_ the policy, or the role that _invokes_ the function (or selects from the view). This is tricky to get right.
