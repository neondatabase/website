...
With a decade of experience working with different types of databases, Leo knew that Postgres would offer them a combination of advantages that’s hard to match. Postgres is a flexible database that (with a bit of config magic) becomes suitable for a wide range of use cases, especially considering [all the Postgres extensions available](https://neon.com/docs/extensions/pg-extensions).
