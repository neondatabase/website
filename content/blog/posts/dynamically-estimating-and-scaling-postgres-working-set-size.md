...
<figcaption><em>Example of updating the HyperLogLog registers after hashing an element.</em><br></br><br></br></figcaption>
</figure>
...
<figcaption><em>Example of updating registers based on timestamps. Instead of setting the bit in the register to “1”, we update the timestamp to the current time, for the first non-zero bit in the remainder of the hash.</em><br></br><br></br></figcaption>
</figure>
...
<blockquote>
<p>“[Aurora Serverless v2] introduces a metric in the engine to estimate the size of the working set in the buffer cache”</p>
</blockquote>

Although they also state the following:

<blockquote>
<p>“Aurora Serverless recommends setting the minimum [capacity] to a value that allows each DB writer or reader to hold the working set of the application in the buffer pool. That way, the contents of the buffer pool aren’t discarded during idle periods.”</p>
</blockquote>
