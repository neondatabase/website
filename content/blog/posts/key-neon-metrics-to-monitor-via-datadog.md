...
We keep working on improving observability for Neon users. A first big step came with the launch of our [monitoring dashboard](https://neon.com/docs/introduction/monitoring-page), and to continue checking boxes, [we’re now launching an integration with Datadog](https://neon.com/docs/guides/datadog) so you can track essential metrics from your Neon databases directly in the Datadog platform.
...
- **Compute metrics** (`host_cpu_seconds_total,host_memory_usage_bytes`): Tracks CPU and memory usage for your compute endpoints.
  - **Why it’s useful:** Helps you optimize your [autoscaling limits](https://neon.com/docs/guides/autoscaling-guide#enable-autoscaling-for-a-compute).
...
Lastly, trends in the working set size over time can inform future resource requirements. For example, a steady increase may indicate growing data usage patterns, telling you that it’s time to [raise your autoscaling limit](https://neon.com/docs/guides/autoscaling-guide#enable-autoscaling-for-a-compute).
...
- **Row operations (INSERTS, UPDATEs, DELETEs).** Monitoring these operations will help you identify tables with heavy write activity, for example, which might inspire some optimization—e.g. by adding a [read replica](https://neon.com/docs/introduction/read-replicas) to make sure performance doesn’t suffer.
...
To make it super easy for you to get started, we’ve put together a [JSON config file](https://neon.com/docs/guides/datadog#dashboard-json) that you can import into Datadog to get a few Neon metrics into a dashboard right away.
...
Follow the instructions [in our docs](https://neon.com/docs/guides/datadog#import-the-neon-dashboard) to import the dashboard and to get all the details about the Datadog integration. If you have any questions, reach out to us ([we’re on Discord](https://discord.gg/92vNTzKDGp)).
