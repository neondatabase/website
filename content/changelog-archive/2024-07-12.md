...
- **Configurable compute sizing with `-cu` option, including support for autoscaling**
  ...
  **Example:**
  ...
  Previously, compute size was determined by your default settings in the console. For more about branching via the CLI, see [Neon CLI commands — branches](/docs/cli/branches).
...
- The Neon CLI `ip-allow` command's `--primary-only` option was deprecated and replaced by a `--protected-only` option. Additionally, in the Neon API, the `primary_branch_only` option in the [Create project](/docs/reference/api/projects/create-project) and [Update project](/docs/reference/api/projects/update-project) methods was deprecated and replaced by the `protected_branches_only` option. The deprecated options will be removed in a future release.
