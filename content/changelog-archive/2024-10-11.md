...
![10 projects on the Neon Free Plan](/docs/changelog/ten_projects.png)
...
![drag-to-zoom monitoring graphs](/docs/changelog/drag-to-zoom.png)

<details>
<summary>**Fixes & improvements**</summary>

- **Neon CLI update**: We released a new version of the [Neon CLI](/docs/cli), with the following updates:
  - Fixed an issue with the `login` alias for the [neon auth](/docs/cli/auth) command.
  - Removed the previously deprecated `set-primary` primary subcommand from the [branches](/docs/cli/branches) command. It was replaced by the [set-default](/docs/cli/branches#set-default) subcommand.
  - Removed the previously deprecated `--allow-list` and `--ip-primary-only` options from the [projects update](/docs/cli/projects#update) command. Operations performed by these options are supported by the [ip-allow](/docs/cli/ip-allow) command.
  - Removed the previously deprecated `--primary-only` option from the [ip-allow](/docs/cli/ip-allow) command. It was replaced by the [ip-allow add --protected-only](/docs/cli/ip-allow#add) option.
  - Updated the [project list](/docs/cli/projects#list) command to output a user-friendly message when there are no projects or shared projects to display.

  To update your Neon CLI installation to the latest version, follow our [CLI upgrade instructions](/docs/cli/install#upgrade).
