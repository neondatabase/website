...
- Each Neon project is initially created with a "root" branch. Previously, this branch was designated as "primary" by default. To simplify our terminology, we've renamed this designation from "primary" to "default." Where you previously saw `primary` in relation to branches in the Neon Console, you will now see `default`. In line with this change, we've added `default` attributes to the Neon API. These will eventually replace the deprecated `primary` attributes in response bodies for endpoints like [Get branches](/docs/reference/api/branches/list-project-branch-endpoints) and [Get branch details](/docs/reference/api/branches/get-project-branch). Additionally, we've introduced a new [Set branch as default](/docs/reference/api/branches/set-default-project-branch) endpoint, which will replace the deprecated Set branch as primary endpoint. The deprecated attributes and endpoint will be removed in a future release.
